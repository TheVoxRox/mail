> **ARCHIV — append-only, zalozeno 2026-06-07** z puvodniho `todo.md` (748
> radku v okamziku rozdeleni). Drzi historii hotovych ukolu, audit reportu a
> implementacnich deniku; nove buckety sem pribyvaji, jak se zavira prace v
> zivem [todo.md](todo.md), ale **jednou zapsany zaznam se uz nereviduje** —
> muze proto obsahovat tvrzeni o kodu, ktera dnes neplati (presne proto archiv
> vznikl). Zive ukoly viz [todo.md](todo.md).
...
- **Zbyva (vyzaduje uzivatele — manualni Windows smoke, plan verification krok 4):** zabalit sidecar, cisty profil → start → overit, ze `crypto.bin` zacina `VOXSEC1` a jeho bytes nejsou plaintext klic; pridat ucet, restart, overit ze sync/login funguje (credentials se desifruji). Pak podstrcit _legacy_ plaintext `crypto.bin` → start → overit in-place migraci na `VOXSEC1` + funkcni credentials.
...
**Stredni priorita (code quality):** 4. ~~**FE deep test audit**~~ — **HOTOVO 2026-06-02 R5d**. Audit 16 `*.test.ts` souboru:
...
6. ~~**`lib/i18n/index.ts` FE testy**~~ — **HOTOVO 2026-06-02 R5b**. 16 testu pokryvajicich SUPPORTED_LOCALES / LOCALE_LABELS expose, `detectInitialLocale` fallback chain (browser=false → DEFAULT, localStorage stored supported / unsupported → navigator, localStorage throw → continues, navigator.language short-code, all-unsupported → DEFAULT), `setLocale` transitions (appLocale derived store, localStorage persistence, `<html lang>` for a11y SR pronunciation, document.title), `syncNativeWindowTitle` Tauri-only path (no-op when non-Tauri, setTitle called when Tauri, rejected setTitle swallowed), `initI18n` idempotent. Pattern: `vi.resetModules` + jsdom env + `Object.defineProperty(window, 'localStorage')` stub. **54.38%/30.76%/53.33%/57.14% → 94.73%/80.76%/100%/95.91%.** Pozn.: svelte-i18n's `locale` store je globalThis singleton ktery prezije vi.resetModules — testy asseruji na _transition_, ne na absolute initial value.
...
**Nizsi priorita (nice-to-have):** 8. ~~**Issue templates**~~ — **HOTOVO 2026-06-02 R5e**. `.github/ISSUE_TEMPLATE/bug.yml` (verze, OS, provider dropdown, repro, expected/actual, logs s redact pripomínkou, search confirmation), `feature.yml` (problem, proposal, alternatives, area dropdown), `config.yml` (`blank_issues_enabled: false` + contact_links na SECURITY policy a CONTRIBUTING.md pres `TheVoxRox/mail` URL). YAML schema validovan pres `js-yaml`. 9. **CODEOWNERS** — `.github/CODEOWNERS` (vyzaduje user GitHub handle: `* @TheVoxRox` nebo podobne). 10. **Full QRESYNC SELECT s VANISHED** — backend refactor pro IMAP cleanup latency (todo:313). Po release prioritizovat — neblokuje V0.1.0.
...
**4 dalsi nalezy opraveny:** 10. **`e2e-helpers.ts` dead block** — knip nalezl `loadBackendSession`, `seedSessionOverride`, `resolveMailFixture` exports bez calleru. Smazany vc. souvisejicich helperu (`fetchApi`, `SESSION_PATH`, `resolveMailDataDir`, `MailFixture`, `SessionPayload` etc.) — 60 radku → 5. 11. **License drift v `THIRD_PARTY_LICENSES.md`** — FE balicek `svelte-toolbelt` 0.10.6 prejmenovan MIT → MIT* (license-checker guess). BE inventory hand-curated 132 vs raw 160 (Spring transitive). Vse regenerovano programaticky. 12. **Backend coverage visibility** — pred dnesnim sweep jacoco se mereno jen unit testy a bez gate. Nyni unit + IT (merged) + bundle threshold gate, pre-push validuje na regresi BE coverage. 13. **Knip false-positives** — pred konfiguraci nasel 8 dead souboru + 1 dead dep + 44 dead exports (vetsina FP — type-contract.ts compile-time guard, test-stubs aliased pres vitest config, UI lib Bits-style re-exports). Po `knip.json` s entries pro routes/e2e/scripts/test-fixtures + `tw-animate-css` CSS import → 0 FP.
...
- [ ] Manualni smoke checklist v `tauri:dev` (rychly check) a `tauri:build`
  ...
  - Tauri okno se zobrazi do 500 ms, NEsmi byt blank screen — musi byt videt
    AppRail placeholder + sidebar placeholder (sedonove obdelnicky).
  - Mereni boot timings: dev console →
    `console.info('boot', $bootState.timings)`.
  - Backend log `logs/mail.log`: hledat
    `[BOOT] Startup timing: phase=spring.application-ready durationMs=...`
    (cilem -20-40 % vs pred AOT).
  ...
  - **Springdoc kontrola**:
    `jar tf backend/target/mail-backend-0.1.0.jar | Select-String springdoc`
    musi byt prazdne (pokud build byl bez `-Popenapi`).
...
# Presunuto z todo.md 2026-07-07 — hotove polozky po zamrazeni archivu
...
## Pred prvnim commitem — manualni smoke (vse hotovo)

Kod byl hotovy + zeleny; tohle vyzadovalo rucni overeni v `tauri:dev`:

- [x] **DPAPI `crypto.bin`** — overeno 2026-06-23 v tauri:dev: cisty profil generuje `VOXSEC1` + DPAPI blob (zadny plaintext); pridani Google+Microsoft uctu → restart → oba uctu desifruji credentials a sync/login bezi bez re-loginu (`token refreshed` z ulozenych refresh tokenu). Format+migrace uz 2026-06-12.
- [x] **Smoke `tauri:dev` pred commitem** — overeno 2026-06-23: (1) IMAP bounded sync bez zaseknuti, (2) OAuth refresh Google i Microsoft (vc. po restartu), (3) bootstrap prazdne DB na V1, (4) SMTP send Google i Microsoft (po MS 587 fixu), (6) edit uctu pres PUT (`[API] Full update (PUT)`), (7) Threading backfill bez chyb.
- [x] **(5) IMAP fetch robustness — Seznam** — pokryto deterministickym IT 2026-06-24, manualni Seznam smoke uz neni blocker. Zive Seznam reprodukce je nedeterministicka (malformed BODYSTRUCTURE Seznam vraci jen obcas) + nemame ucet v CI, takze end-to-end zaruka je zautomatizovana: [MalformedBodyStructureSyncIT](backend/src/test/java/org/voxrox/mailbackend/feature/mail/service/MalformedBodyStructureSyncIT.java) prozene malformed zpravu realnym `MessageDownloader`→`MessageFetcher`→SQLite a overi, ze se ulozi jako envelope-only stub (body=null, hasAttachments=false, envelope intact) a OBJEVI se v listu vedle dobre zpravy (nezahodi se). Produkce stubu na DTO urovni uz kryl [MessageFetcherTest](backend/src/test/java/org/voxrox/mailbackend/feature/mail/service/MessageFetcherTest.java); dynamicky `contentError` na detailu kryje [MailFacadeTest](backend/src/test/java/org/voxrox/mailbackend/feature/mail/service/MailFacadeTest.java). Volitelny bonus: zive overit na seznam.cz uctu.
- [x] **Merge kontaktu** — automatizovano 2026-06-24. Netrivialni dedup/union logika vytazena do ciste [mergePreview.ts](frontend/src/lib/contacts/mergePreview.ts) (case-insensitive dedup, target-first poradi, zachovani primary, hranice 10/11 e-mailu) + unit testy [mergePreview.test.ts](frontend/src/lib/contacts/mergePreview.test.ts) (8 edge-cases). SR ohlaseni preview kryje a11y e2e test v [a11y.e2e.ts](frontend/src/routes/a11y.e2e.ts) (`dialog sloučení kontaktů…`): axe scan dialogu bez porusenii + overeni aria-live="polite"/aria-atomic preview oblasti + pojmenovane radio skupiny. Warning >10 + slouceni dat kryji functional e2e [contacts.functional.e2e.ts:310-394](frontend/src/routes/contacts.functional.e2e.ts).
- [x] **About dialog** — overeno 2026-06-23 v tauri:dev: NOTICE.txt (~100 KB) se nacte pres `resolveResource` do read-only textarea, disclosure toggle + `aria-expanded` OK, zadny error.
- [x] **BootErrorView Retry** — overeno 2026-06-23 v tauri:dev: kill sidecaru → cisty auto-restart (1 backend); vycerpani MAX_RESTARTS → BootErrorView → "Zkusit znovu" nabehne presne 1 backend. Zadny exit 78 / orphan / dvoji spawn (generacni guard [sidecar.ts](frontend/src/lib/backend/sidecar.ts) drzi).
...
- [x] **§6 Sidecar lifecycle: force-kill parenta -> osirely backend JVM (zombie), nalezeno 2026-06-29, opraveno 2026-06-30.** Tvrdy `kill -9` Tauri parenta (`app.exe`) necha sidecar (jpackage launcher + JVM) bezet dal: Windows deti nezabiji a force-kill obejde graceful `stopBackendSidecar`/`beforeunload` ([sidecar.ts](frontend/src/lib/backend/sidecar.ts)), a backend nema watchdog na smrt parenta. Overeno v tauri:dev 2026-06-29: JVM prezila 5+ min a drzela ephemeral port; graceful zavreni je naopak ciste (0 orphanu). **Dopad:** realna konfigurace bezi na ephemeral portu (`DEFAULT_PORT=0` -> `isPortAvailable` vraci rovnou true, [MailBackendApplication.java:80-91](backend/src/main/java/org/voxrox/mailbackend/MailBackendApplication.java)), takze single-instance guard (exit 78) se NEuplatni -> leftover zombie pristi start neblokne a hrozi druhy backend nad stejnou `mail.db` (resp. zombie drzi DB/RAM do rebootu). Odpovida [[reference_sidecar_startup_debugging]] (zombie JVMs). **Opraveno (smer a — stdin-EOF):** backend se sam ukonci pri smrti parenta. Frontend spawnuje sidecar s env `MAIL_SIDECAR_WATCH_PARENT=1` ([sidecar.ts](frontend/src/lib/backend/sidecar.ts)); [ParentProcessWatchdog.java](backend/src/main/java/org/voxrox/mailbackend/core/lifecycle/ParentProcessWatchdog.java) cte `System.in` v daemon vlakne a na EOF/broken-pipe (Tauri zavre stdin pipe pri smrti parenta — graceful i force-kill) zavola `System.exit(0)`. Gatovano env (`mvn spring-boot:run`, testy i build-time AOT ho nenastavuji -> no-op), startovano z `main()` aby pokrylo i 2-5 s startup okno. Job-object (smer b) zamitnut: Windows-only + grandchild race by si vynutil presun spawnu sidecaru do Rustu. Testy: [ParentProcessWatchdogTest](backend/src/test/java/org/voxrox/mailbackend/core/lifecycle/ParentProcessWatchdogTest.java) (5 pripadu) + [sidecar.test.ts](frontend/src/lib/backend/sidecar.test.ts) overuje predani env. **Smoke overen 2026-06-30** na zabalenem sidecaru: (1) izolovany stdin-close — armed=cisty exit za 475 ms, control bez env=zije dal i po 15 s (= puvodni bug); (2) plny tauri:dev, force-kill `app.exe` → oba `mail.exe` (jpackage launcher + JVM grandchild) zmizely za ~13 s, 0 orphanu (z toho ~10 s graceful timeout na viseli SSE, pak cisty Spring/IMAP/Hikari shutdown — integrita SQLite zachovana). Formalni re-smoke jeste na podepsanem release buildu (§6 v RELEASE_CHECKLIST). Pozn.: Test A (kill sidecaru) prosel — re-handshake fix [sidecarRecovery.ts](frontend/src/lib/backend/sidecarRecovery.ts) + [+layout.svelte](frontend/src/routes/+layout.svelte) (#89, merged).
...
## Microsoft OAuth — hotove polozky
...
## Release & Update (Tauri updater) — hotove polozky
...
## Bezpecnost (release-ops) — hotove polozky

- [x] **Produkcni Google OAuth credentials** — HOTOVO 2026-06-23 (verifikovano pres Google Console). Produkce uz bezi na spravnem **org-based** clientu: `VoxRox Mail` (Desktop), client-id `37842026466-…`, v projektu **`VoxRox`** (`lucky-outpost-495609-j5`) pod organizaci **`voxrox.org`** (info@voxrox.org). Zapojeny lokalne v `backend/.env` i v CI secrets `GOOGLE_OAUTH_CLIENT_ID`/`GOOGLE_OAUTH_CLIENT_SECRET` (set 2026-06-17), odkud se peku do launcher cfg ([package-sidecar-windows.ps1:137-141](backend/scripts/package-sidecar-windows.ps1), [windows-signed-release.yml:54-55](.github/workflows/windows-signed-release.yml)). **Nerotuje se** — client uz je in use (rozhodnuto 2026-06-23); git-history secret scan byl cisty. Pozn.: dev client `Google OAuth Test`/`TauriMailClient` (`462204841115-…`) v osobnim gmail uctu vlastnika je pozustatek — nikde zadratovany neni, volitelny budouci uklid (osobni ucet).
  - **Crypto klic/salt = no-op:** `MAIL_CRYPTO_KEY`/`MAIL_CRYPTO_SALT` se NErotuji a neshippuji — generuji se per-machine do DPAPI `crypto.bin` pri prvnim startu ([StorageContextInitializer.java:220-227](backend/src/main/java/org/voxrox/mailbackend/core/init/StorageContextInitializer.java)); kod blokuje zmenu proti existujici DB (fingerprint mismatch, [radky 124-137](backend/src/main/java/org/voxrox/mailbackend/core/init/StorageContextInitializer.java)). Lokalni `.env` je ma prazdne. Jedina relevantni akce = dev clean-start.
  - **Consent screen HOTOVY 2026-06-23** (projekt `VoxRox`, authuser=1): app name `VoxRox Mail`, support email `info@voxrox.org`, **logo** (embossed mark z `C:\dev\java\brand\voxrox-mark.svg` → vyrenderovano do `voxrox-mark-512.png`), home `https://voxrox.org`, privacy `https://voxrox.org/privacy` (zije); **terms odkaz odebran** (vedl na 404 `https://voxrox.org/terms`); authorized domain `voxrox.org`; scopes `email`/`profile`/`openid` + restricted `https://mail.google.com/`. (Zbyvajici Publish + CASA je zivy ukol v todo.md.)
...
- [x] **Zostrena globalni CSP — HOTOVO 2026-06-24.** V [tauri.conf.json](frontend/src-tauri/tauri.conf.json) (oba bloky `csp` i `devCsp`, parita drzena `check:csp`): pridan explicitni `script-src 'self'` a odstranen nepouzity `customprotocol:` placeholder (`default-src` ted `'self' asset:`). `tauri:dev` smoke overen 2026-06-23 pres CDP. **`tauri:build` smoke overen 2026-06-24** novym opakovatelnym skriptem [tauri-csp-build-smoke.mjs](frontend/scripts/tauri-csp-build-smoke.mjs) (`npm run tauri:smoke:csp-build`): packaged `app.exe` spusten s WebView2 remote-debuggingem, pres CDP injektnut `securitypolicyviolation` listener + reload na `http://tauri.localhost/` → inline bootstrap probehl (`__sveltekit_*` global pritomen), SPA se zhydratovala (content div ma deti), **0 CSP violations**.
...
- [x] **CodeQL log-injection (72 medium)** — HOTOVO 2026-06-18, merged v PR [#24](https://github.com/TheVoxRox/mail/pull/24); vsech 72 alertu dismissnuto (0 open). Reseni: globalni Logback guard pres custom converter [CrlfSafeMessageConverter](backend/src/main/java/org/voxrox/mailbackend/util/CrlfSafeMessageConverter.java), ktery prevazuje `%m`/`%msg`/`%message` v [logback-spring.xml](backend/src/main/resources/logback-spring.xml) a stripuje CR/LF na vystupni hranici (kryje vsech ~270 sinku + budouci, console+file+audit). Overeno: 6 unit + 1 integracni test, StartupSmokeTest cisto. Alerty dismissnuty "won't fix — mitigated globally"; CodeQL converter nemodeluje, takze BUDOUCI log-injection alerty se objevi znovu → dismissnout stejne (viz [[reference_github_dismiss_alerts]]).
- [x] **Mail TLS hardening (CodeQL `java/insecure-smtp-ssl`)** — HOTOVO 2026-06-18, merged v PR #25 + #26 (alert #13 → `fixed`, overeno proti post-merge scanu). SMTP [SmtpTransportFactory](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/SmtpTransportFactory.java) STARTTLS vetev nemela `starttls.required` (tichy plaintext downgrade u heslovych uctu) + `checkserveridentity` spolehalo na knihovni default. Fix: explicitni `ssl.checkserveridentity=true` + `starttls.required=true`, parita i pro IMAP [ImapConnectionManager](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/ImapConnectionManager.java) + [MailConnectionProbe](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/MailConnectionProbe.java). **Klicove:** CodeQL property-name queries chteji LITERAL klic — helper s konkatenaci je runtime-OK ale CodeQL ho nevidi (#25 helper nechal #13 open, #26 inline literal ho vyresil), viz [[reference_codeql_property_key_literals]]. Pozn.: plaintext IMAP (`useSsl=false`) zustava moznym dle designu. Zbyle low-prio CodeQL alerty (5,6,86,89,90,91) dismissnuty false-positive/won't-fix; `java/unused-parameter` (`size` v MailSyncService) opraven v kodu.
- [x] Pred releasem projit recovery postup v `OPERATIONS.md` — **Ověřeno 2026-06-30:** DB recovery runbook projit end-to-end na kopii dev profilu — dokumentovany glob `mail.db.backup-pre-v*` sedi, move/copy kroky → backend nabootoval na obnovene DB (`.ready` + `quick_check=ok` + Flyway V1). **Oprava:** doplnen krok smazani `mail.db-wal`/`-shm` pri obnove ([OPERATIONS.md](backend/OPERATIONS.md) §Update troubleshooting) — bez nej hrozila korupce obnovene DB starym WAL. Pozn.: pri stop backendu hned po `.ready` loguje `ThreadingBackfillService` shutdown-race ERROR (async startup task nad zavrenym EM).

## Startup follow-up — hotove analyzy/fixy (2026-06-30)

Defenzivni fixy k manualnimu startup smoke (samotny SR sign-off zustava zivy ukol v todo.md):

- **2026-06-30 — analyza + defenzivni fix:** boot loading je `role="status"` v [BootLoadingView.svelte](frontend/src/lib/components/boot/BootLoadingView.svelte), ten ale ohlasuje jen ZMENY textu, ne pocatecni obsah, a [bootstrap.ts](frontend/src/lib/bootstrap.ts) nevolal zadny explicitni `announce()`. Pridan **explicitni polite announce na startu bootu** (`app.loadingApp`, [bootstrap.ts](frontend/src/lib/bootstrap.ts) po `beginBoot()`) → SR dostane spolehlive okamzite ohlaseni nezavisle na timing chovani role=status. Zelene: check:i18n (571) + svelte-check + 363 unit.
- **2026-06-30 — SR smoke (uzivatel, tauri:dev):** pred oknem ticho, po okne se ozval boot status. Zaver: (a) cast ticha je dev-only (cargo+vite pred oknem, v packaged neni); (b) `app.loadingApp` polite announce se nejspis MINE — prvni aria-live hned po nacteni dokumentu SR casto nezachyti, spolehlivy kanal pri objeveni okna je **titulek okna**. Fix: **loading-aware nativni titulek** — okno se v Rustu ([lib.rs](frontend/src-tauri/src/lib.rs)) vytvori s `VoxRox Mail – načítání…`, [+layout.svelte](frontend/src/routes/+layout.svelte) `$effect` ho prepne na app name az pri boot `ready`/`failed`. Redundantni nativni `setTitle` odebran z [i18n/index.ts](frontend/src/lib/i18n/index.ts), logika vytazena do testovaneho [windowTitle.ts](frontend/src/lib/windowTitle.ts). Zelene: check:i18n (572) + translations:strict + svelte-check + 363 unit + `cargo check`.

## Produktove funkce — hotove polozky

- [x] **Podpisy zprav (per ucet) — Faze 1** — smoke overen 2026-06-23 (auto-insert na nove zprave, From-swap odebere/vrati blok, tlacitko "Vlozit podpis", persist pres PUT vc. diakritiky). Implementovano: sloupec `signature TEXT` ve V1 baseline ([V1__init.sql:120](backend/src/main/resources/db/migration/V1__init.sql) — puvodne V2 migrace, squashnuta do V1 v #53), pole pres [AccountEntity](backend/src/main/java/org/voxrox/mailbackend/feature/account/entity/AccountEntity.java) → [AccountUpdateRequest](backend/src/main/java/org/voxrox/mailbackend/feature/account/dto/AccountUpdateRequest.java) (`@Size(max=10000)`) → [AccountResponse](backend/src/main/java/org/voxrox/mailbackend/feature/account/dto/AccountResponse.java) → [AccountMapper](backend/src/main/java/org/voxrox/mailbackend/feature/account/mapper/AccountMapper.java) → [AccountService.updateAccount](backend/src/main/java/org/voxrox/mailbackend/feature/account/service/AccountService.java); regen OpenAPI snapshot + `schema.d.ts` + rucni [types.ts](frontend/src/lib/types.ts); settings textarea "Podpis" (edit-only) v [AccountForm.svelte](frontend/src/lib/components/AccountForm.svelte) + i18n `accounts.form.signature*` (cs/en); cista [signature.ts](frontend/src/lib/compose/signature.ts) napojena v [ComposeForm.svelte](frontend/src/lib/components/ComposeForm.svelte) (append na nova/mailto, swap pri zmene From, rucni smazani se nevraci, draft/reply/forward netknute) + manualni toolbar tlacitko "Vlozit podpis" ([ComposeActionsBar.svelte](frontend/src/lib/components/compose/ComposeActionsBar.svelte)). **Per-ucet prepinac auto-vkladani** (sloupec `signature_auto_insert BOOLEAN NOT NULL DEFAULT 1` ve V1 baseline [V1__init.sql:121](backend/src/main/resources/db/migration/V1__init.sql) — puvodne V3, squashnuta do V1 v #53): cisty helper `autoSignature(account)` v [signature.ts](frontend/src/lib/compose/signature.ts) ridi onMount append i swap pri zmene From. Testy zelene: AccountMapper + AccountService + FE [signature.test.ts](frontend/src/lib/compose/signature.test.ts) (22). (Zbyva Faze 2 HTML-compose = zivy backlog v todo.md.)
...
# Presunuto z todo.md 2026-07-22 — dalsi hotove polozky
...
## Koncept s rozepsanou adresou nejde ulozit (nalez 2026-07-14) — HOTOVO 2026-07-16 (#166)
...
## Threading Outlook-parity — cross-folder konverzace — HOTOVO 2026-08-06 (#221)

Featura vznikla z feedbacku „seskupeni se nechova jako Outlook". Tri casti: (1) **subject-fallback threading obema smery** — odpoved s `Re:`/`Odp:`/`Fw:` prefixem a BEZ threading hlavicek se pripoji k nejnovejsimu vlaknu se stejnym normalizovanym predmetem v okne +-30 dni, a `reconcileLateArrivingParent` naopak pohlti osirely shluk bezhlavickovych odpovedi, jakmile dorazi jejich rodic (downloader jde od nejnovejsich UID, takze odpoved se pri prvnim syncu skoro vzdy zpracuje driv); novy sloupec `messages.subject_norm` + index (V1 in-place), `SubjectNormalizer` (unicode-aware kvuli NBSP, prazdny sentinel misto NULL), backfill pass. (2) **cross-folder konverzacni vypis** — `messageCount` pocita cele vlakno napric uctem krome kose/spamu/konceptu, kopie jedne zpravy ve vic slozkach jednou; `unreadCount` zustava folder-scoped ve vsech pohledech (oznacit precteno z radku sahne jen na jeho slozku); pohledy Kos/Spam/Koncepty folder-scoped, role pres `ImapFolderService` s fail-closed degradaci. (3) **scope urcuje vyhradne backend** — `GET /threads/{id}?folderRef=` vraci presne tu mnozinu clenu, ze ktere se pocital badge; klient scope neodvozuje, takze badge a rozbalene radky nemuzou divergovat.

Code-review (xhigh): vsech 6 P1, 6 P2 i 6 P3 nalezu opraveno vcetne testu, plus 5 chybejicich testu. Druhe kolo review nad samotnou altitude zmenou: 9 z 10 nalezu opraveno (dedup tie-break srovnany s vyberem reprezentanta ve vypisu, badge se po nacteni clenu odvozuje od nich, prazdny `folderRef` uz neznamena „unscoped", osirely javadoc, `viewFolder` v recordu, `unreadCount` zrcadli radek), jeden stazen jako preceneny.

Z prubezneho smoke vzesly dva a11y nalezy, oba opravene: radek s jedinou zpravou se hlasil jako „Vybrat konverzaci" (ted „Vybrat zpravu", hranice stejna jako u badge), a rozbalovaci caret byl `aria-hidden` span ovladany jen sipkami — pro odecitac tedy nedostupny (browse rezim sipky polyka). Ted je to skutecne `<button>` ve vlastnim sloupci mrizky s `aria-expanded` a nazvem podle stavu; `aria-colcount` 5 → 6. Detail v [[feedback-sr-intercepts-single-key-shortcuts]].

Co pri smoke proti realne schrance sledovat (zustava relevantni i po merge): (a) subject-fallback pri PRVNIM syncu existujici schranky — tam se nejdriv pozna, jestli se vlakna skladaji; (b) badge vs. pocet rozbalenych radku u konverzace, na kterou jsi odpovidal — pokud nesedi, je to signal o resolvovani roli slozek, ne o threadingu; (c) prvni navsteva Kose na cerstvem uctu (fail-closed vetev, folder-scoped); (d) otevreni ciziho clena z rozbaleneho vlakna (URL musi nest jeho slozku).

## Klik mysi na radek posadi fokus do tela zpravy (otazka z ladeni 2026-07-21) — HOTOVO 2026-07-22 (#201)
...
## SR doposlech, code review a tydenni prohlidka (2026-08-31 az 2026-09-04) — uzavreno

Pokracovani predchoziho bucketu: tri uzavrene sekce vystehovane ze ziveho
[todo.md](todo.md), kde zustal jednoradkovy souhrn. Datum + PR drzi stopu;
`file:line` muze byt zastarale.

### SR doposlech — audit Posty — uzavreno

Posledni otevreny bod (badge cizi slozky) doposlechnut 2026-09-04; cela sekce
tim konci. Deterministicke e2e kryti existuje (live-region asserty v [list-navigation.functional.e2e.ts](frontend/src/routes/mail/list-navigation.functional.e2e.ts) a spol.) — tohle je realny NVDA doposlech, ze ohlaseni skutecne zni. Opravy z SR auditu Posty (PR #118, merged 2026-07-06).
...
- [x] **Otevreni radkoveho menu vyvola ctyri fokusy misto dvou** — HOTOVO 2026-09-02, dopad overen sluchem a horsi, nez rikal puvodni zaznam: NVDA cetlo **vsech sedm polozek** menu. Fokus prochazi kontejnerem `role="menu"` (bits-ui ho tam posadi a na prvni polozku se dostane az `afterTick`) a odecitac ho v te mezere popise obsahem. Opraveno pojmenovanim menu **a** synchronnim sundanim fokusu z kontejneru; po oprave `focusin` na kontejneru 0x misto 2x, doposlechnuto — polozka se cte jednou. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Tutez vadu maji jeste tri menu** — HOTOVO 2026-09-02, odposlechnuto, vsechny tri se ted ohlasuji spravne: obe hromadne nabidky a Presunout v otevrene zprave. Ze je vada u vsech tri realna, bylo **overeno reverzi**, ne odvozeno — kazdy test padne na kodu bez opravy se stejnou sekvenci jako u radkoveho menu. Handler je nove sdileny (`focusFirstMenuItem` v [ui/menu](frontend/src/lib/components/ui/menu/menu.ts)) misto ctyr kopii. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Submenu Presunout: fokus v poradku, jmeno chybelo** — HOTOVO 2026-09-03. Otevrena otazka uzavrena **merenim, ne poslechem**, a dopadla jinak, nez polozka predpokladala: `SubContent` fokus kontejneru **nedava**, sekvence pri otevreni sipkou je jedina udalost na prvni polozce. Handler tam tedy nepatri. Vada byla jinde — submenu nemelo pristupne jmeno vubec (ani `aria-label`, ani `aria-labelledby`), jako jedine menu v appce. Opraveno pres `MenuSubContent`, kryto testem, overeno reverzi.
- [x] **Panel nabidky obalen komponentou** — HOTOVO 2026-09-03: `MenuContent` a `MenuSubContent` v [ui/menu](frontend/src/lib/components/ui/menu/), `aria-label` je **povinny prop**, `focusFirstMenuItem` uz z modulu ani neleze ven. Predpoklad o snippetu **overen**, ne tvrzen (typeahead i sipky prochazi). Cestou se nasla **pata nabidka**, ktera handler nikdy nedostala — presne to, co polozka predpovidala. Detail v [CHANGELOG.md](CHANGELOG.md).
...
- [x] **NVDA doposlech seskupeneho rezimu** (threading Phase 2) — HOTOVO 2026-09-04, badge cizi slozky odposlechnut bez nalezu. Cestou se ukazalo, ze stav nesel vyrobit: schranka nemela zadne vlakno pres dve **nevyloucene** slozky, takze se musela jedna zprava presunout z labelu do Dorucenych (a po doposlechu zpet). Zmereno pres AX strom, ne odhadnuto: `aria-hidden` na vizualnim stitku drzi, takze slozku nese jen skryty text v bunce odesilatele. **Vedome uzavrene jako neslysene:** bunka vyberu a bunka akci ciziho clena (obe jmenovane, kazda opakuje jmeno slozky — v nazvu radku je tedy 3x) a dosazitelnost odkazu v browse mode; strom obe ukazuje spravne, ale to je mereni, ne poslech. Detail v [CHANGELOG.md](CHANGELOG.md).
...
- [x] **„Nova zprava" se pri Ctrl+N cte dvakrat** — HOTOVO 2026-09-04: `<form>` prisel o `aria-label`, ktere z nej delalo druhy landmark se jmenem `<h1>` hned uvnitr nej. Pri mereni se ukazalo, ze to jmeno je ve strome **trikrat** (`main` „Posta – Nova zprava", form landmark, `<h1>`), ne dvakrat. Formular kontaktu zamerne zustava pojmenovany — tam jmeno drzi `aria-describedby` s napovedou. Sest testu preslo z `getByRole('form', …)` na `<h1>`, sedme misto byl scope axe scanu. Overeno reverzi. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Presmerovavajici zastavka na `/` ohlasi uvitaci obrazovku, na ktere uzivatel neni** — HOTOVO 2026-09-04: `/` neni misto, ale rozhodnuti „na kterou slozku se Posta otevira", a ted se z nej presmerovava uz v `load`, takze se na nej neda pristat. Puvodni nazev polozky mluvil o Esc z composeru, ale mirilo tam osm volajicich mist a vada byla v rote. Kryto dvema e2e nad zaznamem jmen `<main>`, overeno reverzi. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **NVDA doposlech indikatoru selhavajiciho syncu** (#226) — HOTOVO 2026-09-03, dva vyvolane vypadky site. Bez nalezu: indikator je Tabem dosazitelny, Enter vede do Nastaveni → Ucty a **pristani se ohlasi** (landmark `main` nese „Posta – Nastaveni – Ucty", tedy oprava z 2026-09-02 drzi i tady), po zotaveni zmizi, a toast se cte **jednou** (`aria-live="off"` na `role="alert"` v [ToastRegion.svelte](frontend/src/lib/components/ToastRegion.svelte) drzi). **Dve opravy zapisu:** je to **odkaz** „Synchronizace selhava: <ucet>", ne tlacitko; a „e2e dokazuje fokusovatelnost" je slabsi, nez to znelo — [sync.functional.e2e.ts](frontend/src/routes/sync.functional.e2e.ts) vola `indicator.focus()`, coz o poradi fokusu nerika nic. Osm nalezu nize; sest z nich e2e chytit nemohlo, protoze netvrdi nic o tom, co ohlaseni **rika**.
- [x] **Selhany pruchod se ohlasi jako „Synchronizace dokoncena."** — HOTOVO 2026-09-03: neuplny pruchod ma vlastni zneni `nav.syncFinishedIncomplete`, kryto dvema unity a e2e nad zivou oblasti, overeno reverzi. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Prechod chyboveho stavu se ohlasi znovu se stejnym kodem, kazdych pet minut** — HOTOVO 2026-09-03: porovnava se pritomnost chyby, ne identita kodu, takze prechod selhani→selhani uz neni prechod. Kryto prepsanym a jednim novym testem, overeno reverzi. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Druha asertivni oblast na totez selhani** — HOTOVO 2026-09-03: chyba slozek prestala byt zivou oblasti, chybovy stav seznamu zprav jde z `alert` na `status`. Kryto e2e hledajicim zivou oblast i mezi predky, overeno reverzi. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Zotaveni se nepozna pri pripojeni, ale az dalsim planovanym pruchodem** — HOTOVO 2026-09-03: appka posloucha `online` a spusti pruchod jen pro ucty, o kterych uz vi, ze selhavaji. Kryto e2e sledujicim odchozi POST, overeno reverzi. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Nativni notifikace o selhani syncu neodposlechnute** — HOTOVO 2026-09-04, ale jinak, nez polozka predpokladala. Zapis tvrdil, ze `Notification.permission` je `denied` v **dev profilu**, a chtel doposlech na packaged buildu. Zmereno v bezici Tauri aplikaci pres CDP: `requestPermission()` vrati `denied` **okamzite a bez promptu** i s `userGesture`, `navigator.permissions.query` taky `denied`, a `isSecureContext` je `true` — takze to nedela ani odmitnuti uzivatelem, ani nezabezpeceny origin. Duvod je ve webview vrstve: wry 0.55.1 registruje `PermissionRequested` jen kdyz je zapnuty clipboard a resi v nem vyhradne `CLIPBOARD_READ`, a `NotificationReceived` — kterou by hostitel musel obslouzit, aby se webova notifikace vubec zobrazila — neni ani ve wry, ani v tauri 2.11.5. Kanal tedy na Windows nikdy nefungoval, v zadnem profilu. Kod smazan i se tremi volanimi (selhani syncu, selhani odeslani, nova posta); vsechna tri jen zopakovala vetu toastu, ktery stal hned nad nimi. **Kdyby se desktopove notifikace nekdy chtely**, cesta je `tauri-plugin-notification` (plugin + capability, web API na to nestaci) a pripad, ktery to obhaji, je **nova posta na pozadi** — ne selhani syncu, ktere uz nese toast i trvaly indikator. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Seznam slozek se po zotaveni sam neobnovi** — HOTOVO 2026-09-03: zotavani obnovuje slozky i aktualni stranku seznamu. Kryto e2e, ktery tvrdi pritomnost odkazu na slozku, ne nepritomnost chyby; overeno reverzi. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Zneni chyby synchronizace je pro odecitac nepouzitelne** — HOTOVO 2026-09-03: pricina se klasifikuje (spojeni / prihlaseni) a ma vlastni lokalizovanou vetu bez argumentu, genericke kody prisly o `errorClass` a prestaly opakovat ramec toastu. Overeno reverzi. Detail v [CHANGELOG.md](CHANGELOG.md).
- [x] **Toast: krizky se jmenuji stejne a fokus po zavreni nejde nikam** — HOTOVO 2026-09-03: tlacitko se jmenuje zpravou, kterou zavira, a fokus jde na zbyvajici krizek, jinak zpet tam, odkud do oblasti prisel. Overeno reverzi obou polovin zvlast. Detail v [CHANGELOG.md](CHANGELOG.md).

### Code review sestice commitu (2026-09-02) — uzavreno

Review `885b98a..463a7ac` (#364, #365, #366, #367, #369, #370). Osm nalezu, sedm
opraveno 2026-09-03; osmy se pri doposlechu ukazal jako **neplatny** a jeho oprava
byla revertovana (nize). Kazda oprava ma test, ktery byl **videt spadnout na kodu
bez ni**; detail v [CHANGELOG.md](CHANGELOG.md).
...
- [x] **Routy s autofocusem po umlceni oznamovace neohlasi jmeno route** — UZAVRENO 2026-09-03 **jako neplatny nalez**, doposlechem. Jmeno route se k uzivateli dostane i bez zasahu: NVDA ohlasuje **kontejnery, do kterych fokus vstoupil**, takze pri Ctrl+N zazni „Posta – Nova zprava" z landmarku `main`, pak „Nova zprava, formular" a teprve pak „Komu" — a to i kdyz fokus na `<main>` nikdy nepristane. Oprava (#381) pridavala ctvrte vysloveni tehoz a byla **revertovana**.
  - **Chyba byla v metode mereni, ne v provedeni.** Zaznam pres CDP vidi fokusove udalosti a zapisy do zivych oblasti; ohlaseni pri **vstupu do kontejneru** neni ani jedno, takze v zaznamu chybi — a z „zadny zapis do zive oblasti" se stal zaver „nic route nepojmenuje". Accessibility strom to pritom ukazuje na prvni pohled (`Accessibility.getFullAXTree`: `main` name="Posta – Nova zprava", `form` name="Nova zprava"). Tvrzeni o tom, co odecitac **rekne**, z fokusoveho logu neplyne; tyka se to stejne tak e2e, ktere umi tvrdit o zive oblasti, ne o reci. Viz [[feedback-read-page-not-a11y-tree]].
  - **Rozhodnuto A/B sluchem**, ne usudkem: s hlasenim vypnutym uzivatel „Posta – Nova zprava" slysel dal, se zapnutym ho slysel navic potreti.
...
## Resynchronizovany SELECT a QRESYNC vetev (presunuto z todo.md 2026-09-16)

Obe polozky zavrel #509: Dovecot IT pousti QRESYNC SELECT proti skutecnemu
serveru a oprava otevira slozku s CONDSTORE, bez ktereho Dovecot
HIGHESTMODSEQ neposila. Puvodni zneni beze zmen:

- [ ] **Resynchronizovany SELECT nema cim projit — QRESYNC neinzeruje ani jeden z dostupnych serveru.** Zmereno 2026-09-09 na desktopu proti obema uctum, ctenim post-auth seznamu schopnosti primo z Angusu (docasna sonda v [ImapCapabilities.java](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/ImapCapabilities.java), vracena pres git). Vysledek: **Gmail inzeruje CONDSTORE, ale ne QRESYNC; Outlook / Exchange Online neinzeruje ani jedno**; seznam.cz taky ne, to je ale prevzate z drivejsiho zapisu, dnes nemereno. Zadani znelo „(Gmail, Outlook)" a stalo na predpokladu, ze ty dva QRESYNC inzeruji — takze polozka se s dnesnimi ucty **zavrit neda**.
      Mereni stoji na kontrolni skupine, ne na jednom cisle. U Gmailu vyslo `ENABLE`, `UTF8=ACCEPT`, `MOVE`, `ESEARCH`, `UIDPLUS`, `SPECIAL-USE`, `LIST-EXTENDED` i `CONDSTORE` na `true`, prestoze **zadna z nich neni v pre-auth `CAPABILITY`**, ktery je videt na dratu — sonda tedy cte post-auth seznam a `QRESYNC=false` v nem je vlastnost serveru, ne artefakt cteni spatneho seznamu. Ta zamena byla realne podezreni: jakarta.mail po prihlaseni `CAPABILITY` znovu neposila a post-auth seznam prijima uvnitr potlacene `AUTHENTICATE` vymeny, takze v protokolove stope neni videt. U Outlooku vysly ctyri schopnosti `true` a sedm `false`, takze ani tam nejde o pausalni `false`.
      Dva vedlejsi nalezy. (1) Outlook bez CONDSTORE nikdy neulozi MODSEQ baseline (`last_known_modseq` je null ve vsech peti slozkach), takze jeho slozky se odmitaji uz na „the folder has no MODSEQ baseline or no local UID range yet" a **ke kontrole QRESYNC se vubec nedostanou** — tentyz neviditelny skip, kvuli kteremu vznikl `plainOpenReason` v [ImapFolderExecutor.java](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/ImapFolderExecutor.java). (2) CONDSTORE vetev na Gmailu prokazatelne bezi: cyklus odpracoval `CHANGEDSINCE 3660367 -> HIGHESTMODSEQ 3665883` a nasel 32 zmenenych priznaku v Kosi a 3 ve Spamu.
      Zbyva jedina cesta, jak polozku zavrit: ucet na serveru, ktery QRESYNC opravdu inzeruje. Mezi presety takovy neni a **iCloud je odlozeny** (rozhodnuto 2026-09-09: ne ted, pripadne pozdeji podle zajmu uzivatelu), takze by to musel byt vlastni server pres [CustomServerFields.svelte](frontend/src/lib/components/account-form/CustomServerFields.svelte) — typicky Dovecot. Dokud takovy ucet neexistuje, polozka zustava otevrena bez toho, ze by na ni slo pracovat.
- [ ] **Rozhodnout, co s QRESYNC vetvi, kterou nespusti zadny podporovany preset.** Vzeslo z mereni o polozku vyse a je to rozhodnuti, ne uklid — proto zvlast. `SELECT (QRESYNC ...)` pristal 2026-09-07, ale ze tri presetu, ktere projekt ma, ho neinzeruje ani jeden a iCloud, jediny zbyly kandidat, je odlozeny (ne ted, pripadne podle zajmu uzivatelu). **Mrtvy kod to ale neni:** aplikace umi vlastni IMAP server ([CustomServerFields.svelte](frontend/src/lib/components/account-form/CustomServerFields.svelte), `provider_id` je v schematu nullable) a Dovecot QRESYNC bezne inzeruje, takze uzivatel s vlastnim serverem tu vetev spusti — jen ji nespusti nic, proti cemu umime testovat. Rozhodnuti tedy **neni „nechat, nebo vypnout"**: za prepinac ji davat nedava smysl, vypnul by ji prave tem, kterym funguje. Na vyber zbyva nechat ji s timhle omezenim zapsanym u ni, nebo ji odstranit a vratit se k ni, az bude cim ji overit. Co v obou pripadech plati: **kryji ji testy nad mockem, ne server**, takze „overeno proti serveru" o ni nesmi stat v zadnem dokumentu.
