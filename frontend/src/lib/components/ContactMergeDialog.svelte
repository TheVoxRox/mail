...
<DialogShell {open} onOpenChange={(next) => (next ? onOpenChange(true) : close())} size="xl" scroll>
	<DialogTitle>{$_('contacts.mergeDialogTitle')}</DialogTitle>
	<DialogDescription>
	...
	{:else}
		<fieldset class="mt-4 space-y-2">
			<legend class="text-sm font-medium text-foreground">{$_('contacts.mergeTargetLegend')}</legend
			>
