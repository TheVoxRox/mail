...
class MessageMapperTest {
    ...
    private FetchedMessage createFetched() {
        return new FetchedMessage(42L, // uid
                "Test subject", "John Doe <john@example.com>", "recipient@example.com", "cc@example.com",
                "bcc@example.com", RECEIVED_AT, true, // seen
                false, // flagged
                true, // answered
                "<msg-001@example.com>", "<reply-to@example.com>", "<ref1@example.com> <ref2@example.com>", List.of()); // attachments
    }

    private FetchedMessage createFetchedWithAttachments() {
        ...
        return new FetchedMessage(42L, "Message with attachment", "sender@example.com", "recipient@example.com", null,
                null, RECEIVED_AT, false, true, false, "<msg-002@example.com>", null, null, attachments);
    }
    ...
    class ToEntity {
        ...
        @Test
        @DisplayName("maps every field of the fetched message onto the entity")
        void mapsAllFields() {
            // Arrange
            var fetched = createFetched();
            var account = createAccount();

            // Act
            MessageEntity result = mapper.toEntity(fetched, account, "INBOX", 12345L);
        ...
        void generatesDeterministicStableId() {
            ...
            MessageEntity first = mapper.toEntity(createFetched(), createAccount(), "INBOX", 1L);
            MessageEntity second = mapper.toEntity(createFetched(), createAccount(), "INBOX", 1L);
            ...
            MessageEntity otherFolder = mapper.toEntity(createFetched(), createAccount(), "[Gmail]/All Mail", 1L);
            assertThat(otherFolder.getStableId()).isNotEqualTo(first.getStableId());
        }

        @Test
        @DisplayName("with attachments, maps them onto the entity correctly")
        void withAttachments_mapsThemCorrectly() {
            // Arrange
            var fetched = createFetchedWithAttachments();

            // Act
            MessageEntity result = mapper.toEntity(fetched, createAccount(), "INBOX", 1L);
