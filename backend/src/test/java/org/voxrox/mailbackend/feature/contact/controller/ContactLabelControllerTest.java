...
class ContactLabelControllerTest {
    ...
    void createLabelBlankName() throws Exception {
        ...
        verify(contactLabelService, never()).createLabel(anyLong(), any());
    }
    ...
    void renameUnknownLabel() throws Exception {
        when(contactLabelService.renameLabel(eq(ACCOUNT_ID), eq(99L), any()))
                .thenThrow(new ContactLabelNotFoundException(ACCOUNT_ID, 99L));

        mockMvc.perform(patch("/api/v1/accounts/{aid}/contact-labels/{lid}", ACCOUNT_ID, 99L)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteLabel() throws Exception {
        mockMvc.perform(delete("/api/v1/accounts/{aid}/contact-labels/{lid}", ACCOUNT_ID, LABEL_ID))
                .andExpect(status().isNoContent());
        verify(contactLabelService).deleteLabel(ACCOUNT_ID, LABEL_ID);
    }

    @Test
    @DisplayName("DELETE with a non-positive labelId → 400")
    void deleteLabelInvalidId() throws Exception {
        mockMvc.perform(delete("/api/v1/accounts/{aid}/contact-labels/{lid}", ACCOUNT_ID, 0L))
                .andExpect(status().isBadRequest());
        verify(contactLabelService, never()).deleteLabel(anyLong(), anyLong());
    }

    @Test
    @DisplayName("POST /assignments → 200 with total and changed")
    void assignLabels() throws Exception {
        when(contactLabelService.assignLabels(eq(ACCOUNT_ID), any()))
                .thenReturn(new ContactLabelAssignmentResponse(3, 2));

        var req = new ContactLabelAssignmentRequest(List.of(10L, 11L, 12L), List.of(LABEL_ID), null);
        mockMvc.perform(post("/api/v1/accounts/{aid}/contact-labels/assignments", ACCOUNT_ID)
    ...
    void assignLabelsEmptyContacts() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/accounts/{aid}/contact-labels/assignments", ACCOUNT_ID)
                .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isBadRequest());
        verify(contactLabelService, never()).assignLabels(anyLong(), any());
    }
    ...
    void assignLabelsUnknownContact() throws Exception {
        when(contactLabelService.assignLabels(eq(ACCOUNT_ID), any()))
                .thenThrow(new ContactNotFoundException(ACCOUNT_ID, 99L));
