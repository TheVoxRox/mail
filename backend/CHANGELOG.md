...
- Flyway migrations consolidated a third time, same pre-release rationale: `V2__remote_image_allowlist.sql` (`remote_image_sender` table — the per-sender remote-image opt-in from content-rendering audit finding F2) folded into `V1__init.sql` as section 9, DDL unchanged. Existing dev databases must be recreated: the V1 checksum changed and Flyway validation rejects them.
...
- Log hygiene re-audit (L1, Low): four internal exception messages carried an unmasked e-mail address that flowed into `mail.log` through `GlobalExceptionHandler`'s `Message: {}` WARN line (and the contact ones also into bulk-create per-item results): `AccountAlreadyExistsException`, `DuplicateContactException`, `ContactService.addEmail` and `ContactService.checkNoDuplicatesWithinAccount`. The internal (log-bound) message now masks the address via `LogMasker.maskEmail`; the localized client response still resolves from the message key with the raw argument. Full re-audit record (287 log sites, 68 audit events, frontend stream) in `SECURITY_RELEASE_CHECK.md` §"Log hygiene re-audit (2026-07-10)".
- IMAP/SMTP audit finding B1-1 (Medium DoS, formerly accepted residual AR-3): the message-body fetch is now size-bounded. `MimePartExtractor` reads `text/plain` / `text/html` body parts through the same bounded `getInputStream()` pattern as inline images (`MAX_BODY_BYTES` = 8 MiB transfer-decoded; `mail.<proto>.partialfetch=true` is now pinned explicitly in `ImapConnectionManager` so the chunked-streaming guarantee the cap depends on cannot regress), replacing the unbounded `getContent().toString()` that let a hostile server exhaust the packaged `-Xmx384m` heap with a multi-hundred-MB body. Charset decoding moved behind the cap — an unknown/malformed charset now degrades to UTF-8 with replacement characters instead of failing the message. An oversized body is never buffered or persisted: the new `messages.body_oversize` column (V1 in-place, dev DB reset required) is set best-effort, the detail endpoint serves a localized placeholder (`mail.message.bodyTooLarge`) and every later open short-circuits before IMAP, so the oversized body costs at most one bounded fetch. `multipart/alternative` selection is order-agnostic: an oversized rich part falls back to any plain-text sibling that fits, regardless of part order. Reply/forward drafts quote an oversized original as an empty body via `MailContentService.getOrFetchQuotableContent` — never as the placeholder text. See `docs/IMAP_SMTP_AUDIT.md` §4 and `SECURITY_THREAT_MODEL.md` change log 2.2.
- `multipart/alternative` bodies whose rich alternative is a nested `multipart/related` (the Apple Mail HTML + inline-images layout) now render as HTML. The alternative walk previously matched only direct `text/plain` / `text/html` children, so such messages silently fell back to the plain-text part (or nothing); the new `selectAlternative` recurses into nested multipart alternatives and prefers the richest part that is renderable and within the body cap.
