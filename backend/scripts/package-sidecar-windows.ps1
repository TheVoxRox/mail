...
Invoke-Step "Creating Windows app-image sidecar with jpackage" {
    ...
    #   -XX:+ExitOnOutOfMemoryError         Exit(3) on the first OutOfMemoryError
    #                                       instead of limping on. The default
    #                                       leaves the JVM running with a thread
    #                                       killed mid-operation -- here that can
    #                                       be a sync thread holding a lane lock,
    #                                       so the account never syncs again and
    #                                       nothing says why. The frontend's
    #                                       sidecar supervisor respawns the
    #                                       process (sidecar.ts), so the exit is
    #                                       what recovery looks like rather than
    #                                       a lost session.
    #
    # One list rather than a literal per --java-options, so the verification step
    # below reads what the launcher was built from. What that proves is not that
    # this script spelled the same flag twice -- it is that jpackage wrote every
    # option into the .cfg and that nothing between here and the published image
    # dropped one.
    $script:jvmOptions = @(
    ...
    $jvmOptionArgs = @()
    foreach ($option in $script:jvmOptions) {
        $jvmOptionArgs += "--java-options"
        $jvmOptionArgs += $option
    }
    ...
    # class cache" step and copied into app/. Kept out of $jvmOptions because it
    # is conditional, and the verification step asserts a list that must be
...
Invoke-Step "Verifying JVM options in launcher" {
    # The tuning above only exists once it is in the published .cfg, and an
    # option that fails to arrive there is invisible: the sidecar starts, serves
    # mail and passes every test without it. -Xmx384m and -XX:+UseSerialGC would
    # cost memory quietly; -XX:+ExitOnOutOfMemoryError would cost nothing at all
    # until a heap exhaustion in the field, where it is the difference between a
    # process the supervisor restarts and one that keeps running with a sync
    # thread killed mid-lane (audit B1-1, B1-3, B1-7).
    #
    # The .cfg is read after the publish step, not out of the jpackage work dir,
    # because that is the file that ships.
    $cfgPath = Join-Path $sidecarDir "app\$sidecarName.cfg"
    if (-not (Test-Path -LiteralPath $cfgPath)) {
        throw "Launcher config not found for JVM option verification: $cfgPath"
    }
    # A .cfg option is one whole line ("java-options=-Xmx384m"), so match the
    # line rather than the substring: -Xmx384m is also a prefix of -Xmx384mb.
    $cfgLines = @(Get-Content -LiteralPath $cfgPath)
    $missing = @($script:jvmOptions | Where-Object { $cfgLines -notcontains "java-options=$_" })
    if ($missing.Count -gt 0) {
        throw "Launcher $cfgPath is missing $($missing.Count) JVM option(s): $($missing -join ', '). The sidecar would run untuned; check the jpackage step."
    }
    Write-Host "  All $($script:jvmOptions.Count) JVM options present in the launcher."
}
