...
- [ ] **§3 Fresh install** — on the signed build from the new tag. Run Release Candidate Smoke against the draft first and record its run URL here; it is the evidence for the items the note at the top of §3 lists. Before the manual part, run the WebView2 install-date check the `a2caa27` sheet describes, so the no-window finding gets its evidence this time.
