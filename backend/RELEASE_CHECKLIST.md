...
- **Waiting for the next cut: #570** (a delete from the grouped list's row menu or bulk bar names the message or conversation it deleted, as a delete of the open message does). This cut takes everything that had accumulated: the fixes and repo-only changes the 2026-09-21 sheet listed, plus #566, #567 and #568, and the two dependency bumps (#535, #538) whose versions `regen:licenses:all` has now moved into `NOTICE.txt`. Each fix that merges from here adds its number here, and the re-cut that carries it comes **before §3 starts**: manual evidence taken on the current tag would be discarded with it.
