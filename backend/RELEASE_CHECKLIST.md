...
- **Waiting for the next cut: #543** (a hint is read once, with its control). Fixes are batched into one re-cut (maintainer, 2026-09-21), and that re-cut comes **before §3 starts**: manual evidence taken on the current tag would be discarded with it. Each fix that merges before then adds its number here.
