...
- [ ] `mvn -Dapp.data-dir=target/test-data clean verify` — one command, not `spotless:check` + `package` separately. `clean` because otherwise SpotBugs analyses the `__BeanDefinitions` left by an earlier `-Paot package` and fails on generated code; `verify` because `package` does not fire the failsafe integration tests. Spotless and SpotBugs both run inside it. From PowerShell, quote the `-D` argument (`'-Dapp.data-dir=target/test-data'`): unquoted, PowerShell splits it at the first dot and Maven stops on the unknown lifecycle phase `.data-dir=target/test-data` before building anything.
...
### Candidate 2026-09-23 — tag `v0.1.0` (see the object ids below)
...
| path                         | object id      |
| ---------------------------- | -------------- |
| `backend/src`                | `a57ad9838d6f` |
| `backend/pom.xml`            | `63ece934432b` |
| `backend/scripts`            | `6cd5fc47cf45` |
| `frontend/src`               | `6985bc90b043` |
| `frontend/src-tauri`         | `afdebca536ba` |
| `frontend/package.json`      | `7152aea39025` |
| `frontend/package-lock.json` | `32fc84ef6308` |

- [x] **§0 Version** — `npm run check:versions` OK, `0.1.0` on all five files; unchanged by this cut, which re-takes the same version.
- [x] **§1 Backend build** — `clean verify` green: surefire 1373 and failsafe 101, 0 failures and 0 errors on both. The one skip is `MailSyncQresyncDovecotIT` ("disabledWithoutDocker is true and Docker is not available"), the local condition #511 documents; CI runs it. `mail-backend-0.1.0.jar` produced; sidecar packaged with the launcher, `app/`, `runtime/` and the production Google client-id — the packaging step’s own checks report "Google client-id baked into the launcher; no placeholder", all 8 JVM options present in the launcher (the check #562 added), and 14 of 14 modules declared for the runtime. `regen:licenses:all` ran first and moved the four components the dependency bumps carry (`@tauri-apps/plugin-http` 2.6.1, `tailwind-merge` 3.7.0, `@jridgewell/sourcemap-codec` 1.6.0, `yaml` 2.9.1), which the inventories and `NOTICE.txt` now carry; they match `package-lock.json`, so the regen read an installed tree in step with it. **Open:** the run on a clean profile without a system JDK.
- [x] **§2 Frontend automation** — `generate:api` (regenerated `schema.d.ts` with no drift), `check:i18n` (750 keys), `build`, and `test:e2e`, which is the wrapper that runs the functional project (306 passed) and then the a11y suite (68 passed). All green, no flake and no re-run.
- [ ] **§3 Fresh install** — open, including the machine half: the Release Candidate Smoke of the previous cut belonged to the build of `a45e905` and is gone with its sheet. Take the first finding below before the manual half.
- [ ] **§3a Installer behaviour** — open. By hand: no UAC prompt (runners are administrators), the privacy page with a screen reader, `latest.json` read from the draft by hand, a reinstall over an installation that holds an account, and the downgrade block, which no cut has covered.
- [ ] **§4 Account flows** — open. The Google login is the blocking item: it proves the production client-id is in the launcher `.cfg`.
- [ ] **§5 Mail workflows** — open, including a trash folder holding two IMAP copies of one Message-ID (both persist, `lastSyncAt` is set), the tray items "Synchronizovat" and "Nová zpráva" (#490), printing (Ctrl+P on an open message, the bulk bar’s "Vytisknout vybrané" on ticked messages, and Ctrl+P with nothing open or ticked, which prints nothing and says so; #530, #532), and what is new in this cut: the conversation view’s own delete path — the announcement names the message, a delete in the trash asks, the list and the folder badge follow it and the reading cursor lands on the thread (#566, #567) — and Ctrl+U right after opening a message, which keeps it unread (#568).
- [ ] **§6 Sidecar lifecycle** — open. By hand: Task Manager lists both processes, killing only the sidecar and restarting it from the UI, and the damaged-database boot error view.
...
- **Waiting for the next cut: #570** (a delete from the grouped list's row menu or bulk bar names the message or conversation it deleted, as a delete of the open message does), **#571** (internal: the IDE no longer runs Spotless on save; `backend/pom.xml` changes, the build does not). This cut takes everything that had accumulated: the fixes and repo-only changes the 2026-09-21 sheet listed, plus #566, #567 and #568, and the two dependency bumps (#535, #538) whose versions `regen:licenses:all` has now moved into `NOTICE.txt`. Each fix that merges from here adds its number here, and the re-cut that carries it comes **before §3 starts**: manual evidence taken on the current tag would be discarded with it.
