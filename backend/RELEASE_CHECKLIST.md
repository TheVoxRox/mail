...
### Candidate 2026-09-21 — tag `v0.1.0` (see the object ids below)
...
| path                         | object id      |
| ---------------------------- | -------------- |
| `backend/src`                | `751f7af40d6e` |
| `backend/pom.xml`            | `63ece934432b` |
| `backend/scripts`            | `bd35a3977876` |
| `frontend/src`               | `04033d324e17` |
| `frontend/src-tauri`         | `474a35a235ef` |
...
- [x] **§1 Backend build** — `clean verify` green: surefire 1349 and failsafe 94, 0 failures and 0 errors on both. The one skip is `MailSyncQresyncDovecotIT` ("disabledWithoutDocker is true and Docker is not available"), the local condition #511 documents; CI runs it. `mail-backend-0.1.0.jar` produced; sidecar packaged with the launcher, `app/`, `runtime/` and the production Google client-id — the packaging step's own check reports "Google client-id baked into the launcher; no placeholder", 0× `mail-local-`, and the `.cfg` also carries the Google client secret and the Microsoft client-id. `regen:licenses:all` ran first, on a fresh `npm ci`, and moved no component or version, so no inventory is committed. **Open:** the run on a clean profile without a system JDK.
- [x] **§2 Frontend automation** — `generate:api` (regenerated `schema.d.ts` with no drift), `check:i18n` (745 keys), `build`, and `test:e2e`, which is the wrapper that runs the functional project (288 passed) and then the a11y suite (65 passed). All green, no flake and no re-run.
- [ ] **§3 Fresh install** — machine half green: Release Candidate Smoke on the signed build of this tag (installer from `refs/tags/v0.1.0` at `a45e905` by provenance), 22 of 22 checks, [run 35571052973](https://github.com/TheVoxRox/mail/actions/runs/35571052973). It covers the silent per-user install and what it carries, both shortcuts, a first start to ready in 18 s, `session.json`, health 200, `crypto.bin`, the WebView2 profile and the Tauri log under `VoxRoxMail`, no `org.voxrox.mail*` folder, the webview at boot phase ready, and a second start over the same `crypto.bin`. Open by hand: a first start while WebView2 is still installing, which a runner cannot see. Take the first finding below before it.
- [ ] **§3a Installer behaviour** — open. The same smoke run covers two items: the silent install records no answer about the startup update check, and a silent reinstall of the same version keeps `crypto.bin`. By hand: no UAC prompt (runners are administrators), the privacy page with a screen reader, `latest.json` read from the draft by hand, a reinstall over an installation that holds an account (the smoke's holds none), and the downgrade block, which no cut has covered.
- [ ] **§4 Account flows** — open. The Google login is the blocking item: it proves the production client-id is in the launcher `.cfg`.
- [ ] **§5 Mail workflows** — open, including a trash folder holding two IMAP copies of one Message-ID (both persist, `lastSyncAt` is set), the tray items "Synchronizovat" and "Nová zpráva" (#490), and printing, which is new in this cut: Ctrl+P on an open message, the bulk bar's "Vytisknout vybrané" on ticked messages, and Ctrl+P with nothing open or ticked, which prints nothing and says so (#530, #532).
- [ ] **§6 Sidecar lifecycle** — open. Killing `voxrox-mail.exe` alone takes the sidecar with it on the smoke run above, gone 10.8 s after the kill. By hand: Task Manager lists both processes, killing only the sidecar and restarting it from the UI, and the damaged-database boot error view.
...
- **Nothing manual carries from earlier cuts, and this is the sixth.** `v0.1.0` was first tagged on `db11430`; only that first build was taken by hand, and it is gone. The fifth cut, `b503ea2`, had nothing manual on it either, which is what made this one free. The reasons for each cut are in `CHANGELOG.md` under 0.1.0.
- **Every fix goes into 0.1.0 for now (maintainer, 2026-09-21), and the merge freeze stays suspended.** A merge moves `main`, not the tag, so what merges reaches 0.1.0 only through another cut. Each cut costs the machine half (§0–§2, the signed build and the smoke) while nothing from §3 on is ticked, and every ticked manual section after that — which is when this is worth deciding again.
- **Waiting for the next cut: #543** (a hint is read once, with its control), **#548** (a failed save or send leaves focus where it started, not in the To field), **#550** (a screen reader starts reading the new page at its heading after a navigation), **#552** (the follow-ups to the review of that day: the bulk bar keeps focus while an action runs, dialogs and notifications stay off paper, a print that cannot start lets go), **#553** (a message still loading or failed to load is not printed as its placeholder), **#554** (manual account setup announces the provider it found, or that the domain is unknown), **#555** (the message detail no longer fetches the body, so the header shows without waiting for the mail server and a failed body is reported once), **#556** (Ctrl+P from the open message's Move or More menu prints that message, not the ticked rows), **#557** (internal: the sync keeps its own record of a fetched message, no behaviour change), **#560** (an IMAP account with SSL off is encrypted with required STARTTLS instead of logging in over cleartext; audit finding B1-4), **#558** (a screen reader starts at the folder heading after a folder switch, JAWS included, and a navigation from the palette lands on the new page's heading), **#561** (a mail server can no longer exhaust the backend's memory with a message count or a range of vanished UIDs in its answer to a folder open; audit finding B1-3), **#562** (the sidecar stops itself on an exhausted heap and the app restarts it, instead of serving on with a thread killed mid-operation), **#563** (a mail server can no longer exhaust the backend's memory by announcing how long a response is; audit finding B1-7), **#564** (sending a draft you did not edit no longer trusts who the server says it is addressed to, and the fetch of that copy is bounded; audit finding B1-5), **#565** (a mail server that stops reading no longer stalls the account writing to it; audit finding B1-6, the last one open), **#566** (in the conversation view: a deleted message is named in the announcement, a delete in the trash asks before it purges, and closing an open message returns to its folder), **#567** (the conversation view's list, folder badge and reading cursor all follow a delete of the open message), **#568** (Ctrl+U right after opening a message keeps it unread instead of being dropped), and two dependency bumps that change what the installer carries: #535 (`@tauri-apps/plugin-http` 2.6.1, `tailwind-merge` 3.7.0) and #538 (`tauri-plugin-http` 2.6.1, its Rust half). §1's `regen:licenses:all` will move their versions in `NOTICE.txt`. #536, #545, #546 and #559 also merged since the cut and touch only the repo: dev tools, the dependabot config, a gate's test and the IMAP/SMTP audit's re-verification. Fixes are batched into one re-cut (maintainer, 2026-09-21), and that re-cut comes **before §3 starts**: manual evidence taken on the current tag would be discarded with it. Each fix that merges before then adds its number here.
