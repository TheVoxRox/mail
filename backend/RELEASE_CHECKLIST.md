...
- **The merge freeze is suspended (maintainer, 2026-09-21).** It let only a release blocker merge until §3–§9 were finished, on the grounds that each merge invalidates the build the manual sections are taken on. A merge moves `main`, not the tag, and the candidate is the tag: #529–#533 merged after the cut and `v0.1.0` still stands at `b503ea2`. What merges now goes to 0.1.1 unless the tag is re-cut, and a re-cut is its own decision — it costs the machine half while nothing from §3 on is ticked, and every ticked manual section after that.
