...
- [ ] **Verified publisher / MPN ID** zaregistrován a propsán do Azure registrace. Bez něj externí uživatelé z cizích Entra tenantů dostanou AADSTS700016 (reprodukováno 2026-05-20 s `@tyflocentrum-ol.cz`); MSA účty (`@outlook.com`/`@hotmail.com`/`@live.com`) a vlastní tenant účty (`@voxrox.org`) projdou i bez něj. **Toto je blokující bod pro produkční consumer release.**
...
- [ ] Zkontrolovat `logs/mail-backend.log` bez neočekávaných `ERROR`.
