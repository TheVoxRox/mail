...
| **I** | Inline images exfiltrate via tracking pixels | Remote image as user fingerprint | Medium | Remote images are unconditionally stripped by [content-sanitizer.ts](frontend/src/lib/mail/content-sanitizer.ts) — only inline `data:image/*;base64` survives `isInlineImage`; a remote `<img>` loses its `src` and is then removed. Reinforced by the body frame's `<meta>` CSP `img-src data:` and its opaque-origin `sandbox="allow-scripts"` (no `allow-same-origin`, no network) ([mailFrame.ts](frontend/src/lib/mail/mailFrame.ts)). There is no remote-image-loading toggle, so there is no opt-in exfil path |
