...
see [docs/IMAP_SMTP_AUDIT.md](docs/IMAP_SMTP_AUDIT.md). One Medium DoS gap
(B1-1, unbounded body fetch) is recorded as an accepted residual (AR-3).
...
| **D** | Oversized message body exhausts the sidecar heap | Hostile/compromised server sets a multi-hundred-MB body the user then opens | Medium | MIME recursion (`MAX_DEPTH=20`), References walk (`MAX_REFERENCES_WALK=50`) and inline images (2 MiB/image, 8 MiB/message) are bounded; the body text itself is **not** yet capped (B1-1). Accepted residual **AR-3** — self-inflicted local DoS, recoverable by restart, no persistent damage; see [docs/IMAP_SMTP_AUDIT.md](docs/IMAP_SMTP_AUDIT.md) |
...
| **AR-3** | Message body fetch is not size-bounded | A hostile/compromised/MITM'd mail server can set any message to carry a multi-hundred-MB body; when the user **opens that message**, `MailContentService` buffers it into a `String` and again through Jsoup, and can exhaust the packaged `-Xmx384m` heap. Bodies are lazy-fetched (not pulled during sync), so a large mailbox alone is safe. | The precondition is a semi-trusted party (a mail server the user chose to connect to) plus explicit user interaction, and the damage is a local OOM the parent-process watchdog recovers from — the poisoned body is never persisted (OOM precedes the DB write), so there is no permanent denial, data exposure, integrity loss, or code execution. Closing it well needs a truncate-and-offer-original UX (a product decision), not a one-liner. Discovered in the Boundary 1 audit (finding B1-1). | **Now:** the recursion/References/inline-image caps already bound every *structural* amplification; only the flat body text is uncapped. **Upgrade path:** read the body part through a bounded `getInputStream()` (5–10 MiB decoded) and, on overflow, store a "message too large to display — download original" placeholder (mature-client behaviour). See [docs/IMAP_SMTP_AUDIT.md](docs/IMAP_SMTP_AUDIT.md) §4. |
