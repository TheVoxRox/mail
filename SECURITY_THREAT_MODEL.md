...
| **Hostile updater feed** | Hijacks `latest.json` or installer download URL. | Yes — Tauri Ed25519 signature on the update package. The installer itself is not Authenticode-signed (see AR-4). |
...
Audited 2026-07-09 (full tier, verified against `35a06f3`) — verdict **PASS**,
see [docs/IMAP_SMTP_AUDIT.md](docs/IMAP_SMTP_AUDIT.md). The audit's one Medium
DoS gap (B1-1, unbounded body fetch) was briefly carried as accepted residual
AR-3 and **closed in code on 2026-07-10** — the body read is now byte-capped
(see the second D row).

| ST | Threat | Vector | Severity | Mitigation |
|---|---|---|---|---|
| **S** | Spoofed mail server | Hostile DNS / MITM steers user to a fake IMAP server | High | TLS certificate validation with explicit `ssl.checkserveridentity=true` on the IMAP store, SMTP transport and credential probe (not left to the JavaMail default); the OAuth defense-in-depth blocks the access token from ever travelling over a plaintext socket — SMTP via `requireSslForOAuth2` in [SmtpTransportFactory.java](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/SmtpTransportFactory.java), IMAP via the fail-fast CRITICAL `imap_oauth2_plaintext_blocked` guard in [ImapConnectionManager.java](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/ImapConnectionManager.java) |
| **T** | Mail tampered in transit | Active MITM modifies fetched mail | Medium | TLS as above; UID + UIDVALIDITY cross-check between fetches detects mailbox-level desync ([FlagSyncService.java](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/FlagSyncService.java) resets sync state on a UIDVALIDITY change) |
| **I** | Plaintext credentials over a downgraded connection | Forced STARTTLS strip | **Critical** | SSL/TLS enforcement; `mail.smtp.starttls.required=true` (a stripped upgrade fails, never sends cleartext); the IMAP probe in [MailConnectionProbe.java](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/MailConnectionProbe.java) rejects OAuth2 + non-SSL combination *before* opening the socket |
| **D** | Slow / unresponsive server hangs sidecar | Bad server / saturated link | Low | Per-protocol read & connection timeouts (60 s / 30 s for IMAP, connect 30 s for SMTP) configured through `mail.imap.*` / `mail.smtp.*` and correctly passed as millisecond strings — fixed in Phase 6.15 (the values used to be ignored by JavaMail) |
| **D** | Oversized message body exhausts the sidecar heap | Hostile/compromised server sets a multi-hundred-MB body the user then opens | Medium | Every read is bounded: MIME recursion (`MAX_DEPTH=20`), References walk (`MAX_REFERENCES_WALK=50`), inline images (2 MiB/image, 8 MiB/message) and — since the B1-1 fix (2026-07-10) — the body text itself (8 MiB via a bounded `getInputStream()` read in [MimePartExtractor.java](backend/src/main/java/org/voxrox/mailbackend/util/MimePartExtractor.java)). An oversized body is never buffered or persisted: the `messages.body_oversize` flag is stored, a localized placeholder is served, and later opens skip IMAP entirely; see [docs/IMAP_SMTP_AUDIT.md](docs/IMAP_SMTP_AUDIT.md) §4 |
...
| **AR-4** | Installer not Authenticode-signed | A **first install** has no OS-verified publisher: SmartScreen shows the "Unknown publisher" warning and Windows reputation is absent, so a user fetching the installer from anywhere but the official releases page could be socially engineered into running a tampered build. **Updates are unaffected** — the Ed25519 updater signature is verified natively regardless of Authenticode (Boundary 6). | A paid code-signing certificate is not justified for this open-source project (decision 2026-07-18, recorded in `todo.md`); the repo holds no `WINDOWS_CERTIFICATE_BASE64` secret, so `windows-signed-release.yml` builds unsigned by design. Exposure is bounded to first installs obtained outside the HTTPS GitHub releases page. | **Now:** Sigstore build-provenance attestation on every release (`gh attestation verify <file> --repo TheVoxRox/mail`) + `.sha256`; users are told the build is unsigned and how to verify it in [frontend/END_USER_README.md](frontend/END_USER_README.md) and [frontend/docs/WINDOWS_SIGNING.md](frontend/docs/WINDOWS_SIGNING.md). **Upgrade path:** free OSS signing (SignPath.io OSS program, Azure Trusted Signing if eligibility lands) or an OV cert if a legal entity emerges — the workflow signs automatically once the secrets exist; this would also strengthen AR-2's launch-time sidecar integrity option. |
...
| 2.1 | 2026-07-09 | Boundary 1 (external mail server) promoted from an informal review to a **full** audit — verdict **PASS**, verified against `35a06f3`, see [docs/IMAP_SMTP_AUDIT.md](docs/IMAP_SMTP_AUDIT.md). Confirmed: explicit `checkserveridentity` hostname verification on IMAP/SMTP/probe; OAuth2-token-over-cleartext fail-closed on **both** protocols (SMTP `requireSslForOAuth2` + IMAP CRITICAL `imap_oauth2_plaintext_blocked`); mandatory STARTTLS; effective timeouts; MIME depth (20), References walk (50) and inline-image bytes (2/8 MiB) all bounded; malformed BODYSTRUCTURE fails soft to an envelope stub; UIDVALIDITY cross-check; `From` is display-only data and its one security-load path (remote-image allow-list) affects image loading only. Added the B1 `D`-row for the audit's one finding **B1-1** (unbounded body fetch → heap DoS) and recorded it as accepted residual **AR-3** with a recommended truncate-and-offer-original fix. All six boundaries now hold a full or focused verdict. |
