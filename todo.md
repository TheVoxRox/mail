...
- [ ] **§6 Sidecar lifecycle: force-kill parenta -> osirely backend JVM (zombie), nalezeno 2026-06-29.** Tvrdy `kill -9` Tauri parenta (`app.exe`) necha sidecar (jpackage launcher + JVM) bezet dal: Windows deti nezabiji a force-kill obejde graceful `stopBackendSidecar`/`beforeunload` ([sidecar.ts](frontend/src/lib/backend/sidecar.ts)), a backend nema watchdog na smrt parenta. Overeno v tauri:dev 2026-06-29: JVM prezila 5+ min a drzela ephemeral port; graceful zavreni je naopak ciste (0 orphanu). **Dopad:** realna konfigurace bezi na ephemeral portu (`DEFAULT_PORT=0` -> `isPortAvailable` vraci rovnou true, [MailBackendApplication.java:80-91](backend/src/main/java/org/voxrox/mailbackend/MailBackendApplication.java)), takze single-instance guard (exit 78) se NEuplatni -> leftover zombie pristi start neblokne a hrozi druhy backend nad stejnou `mail.db` (resp. zombie drzi DB/RAM do rebootu). Odpovida [[reference_sidecar_startup_debugging]] (zombie JVMs). **Smery fixu:** (a) backend self-terminuje pri smrti parenta — EOF na stdin (Tauri sidecar pipe se zavre, kdyz parent umre) nebo poll liveness parenta -> `System.exit`; (b) Tauri/Windows job-object (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`) na sidecar, aby deti umrely s parentem. Vaze se na §6 Test B v [backend/RELEASE_CHECKLIST.md](backend/RELEASE_CHECKLIST.md) (radky 145-146). Pozn.: Test A (kill sidecaru) prosel — re-handshake fix ([sidecarRecovery.ts](frontend/src/lib/backend/sidecarRecovery.ts) + watcher zapojeny v [+layout.svelte](frontend/src/routes/+layout.svelte)) na branchi `fix/sidecar-restart-rehandshake` (zatim nemergovano).
