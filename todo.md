...
- [ ] **Clenske radky vlakna porad opakuji predmet konverzace** — v bunce predmetu i v nazvu spoustece menu akci, takze rozbalene vlakno precte tentyz predmet tolikrat, kolik ma zprav. Je to zbyla polovina toho, co resila oprava rodicovskeho radku (`feat/conversation-row-trim`): ta sundala duplicitu odesilatele a data nahore, tohle je duplicita predmetu dole. Zamysleny krok: clensky radek nese odesilatele + cas misto predmetu, menu akci pojmenovat podle nich. Pozor na zavisle testy — `grouping.functional.e2e.ts` dnes clenske radky adresuje mimo jine pres jejich predmet.
- [ ] **Plochy seznam zprav ma porad per-radkovy grid** — [MessageList.svelte](frontend/src/lib/components/MessageList.svelte) drzi `grid-cols-[2.5rem_auto_minmax(0,1fr)_auto_auto]` na kazdem radku, takze oba `auto` tracky se dimenzuji podle obsahu toho radku a sloupce mezi radky nelicuji, kdyz se formaty lisi: zmereno 2026-08-17 v seskupenem seznamu pred opravou 1164 px pro „14:32", 1148 pro „Ctvrtek", 1118 pro „31. 12. 2026". Seskupeny seznam to uz resi sdilenym sloupcem (kontejner grid + `grid-cols-subgrid` na radcich, viz ConversationList), tady zbyva totez prenest. Pozor na tri veci, vsechny zmerene: (1) kontejner potrebuje `content-start` — bez nej se implicitni `auto` radky roztahnou, jediny radek naskocil z 65 px na 597 px, tedy na celou vysku podokna; (2) jakykoli padding na radku musi napred prejit do bunek, protoze pod subgridem posunuje vsechny tracky radku a posledni sloupec utece za pravy okraj (v `MessageList` je dnes `padding: 0px`, takze tahle past tam zatim spi — vadila u odsazeni clenskych radku v seskupenem seznamu); (3) **pocet tracku se neridi `aria-colcount`** — `MessageList` hlasi 6 sloupcu, ale CSS tracku ma pet (`40px 30px auto auto auto`, predmet a odesilatel sdileji treti ve dvou radkach), stejny nepomer ma i seskupeny seznam (7 ku 6). Napsat do kontejneru pocet podle ARIA je tichy omyl: mrizka se nerozbije napadne, jen se vsechno posune o sloupec. Rozsah problemu: tri radky se tremi formaty data meri levou hranu data 788 / 772 / 742 px, tedy rozptyl 46 px mezi sousedy; sloupec akci drzi, protoze ho `1fr` predmetu dorovna.
