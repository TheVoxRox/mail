...
Nalezeno pri rucnim release smoke v0.1.0 (signed build, cisty profil `%LOCALAPPDATA%\VoxRox\Mail`). Opraveno: FE update-dialog (#65), OAuth add-account poll reconcile (#66), "duch" zpravy 404 A+B (deterministicky stableId z message-id [MessageStableId.java](backend/src/main/java/org/voxrox/mailbackend/feature/mail/mapper/MessageStableId.java) + FE graceful 404 → reload seznamu [selectedMessage.ts](frontend/src/lib/stores/selectedMessage.ts)), C SSE 30min ERROR (permit ASYNC/ERROR dispatch v [SecurityConfig.java](backend/src/main/java/org/voxrox/mailbackend/core/config/SecurityConfig.java)), F soubezny sync UNIQUE constraint ERROR (idempotentni insert — `findExistingUids` filtr v [MessageDownloader.java](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/MessageDownloader.java)), G HHH90003004 in-memory paginace (#70 — skutecny zdroj byl contact list, NE list zprav: `@EntityGraph(emails)` + `Pageable` v [ContactRepository.java](backend/src/main/java/org/voxrox/mailbackend/feature/contact/repository/ContactRepository.java); fix = drop `@EntityGraph` z paginovanych finderu + `@BatchSize` na [ContactEntity.emails](backend/src/main/java/org/voxrox/mailbackend/feature/contact/entity/ContactEntity.java), regresni IT capturuje query logger), E duplicitni OAuth callback → matouci `auth-failed.html` i po uspesnem pridani (#71 — `oauthStarting` guard + `disabled` login tlacitko v [+page.svelte](frontend/src/routes/settings/accounts/new/+page.svelte) pusti jen jeden flow; `auth-failed.html` vedome NEzmekcen; smoke overen 2026-06-25 v tauri:dev — pridani Google uctu probehlo bez auth-failed). Zde jen otevrene:

- [ ] **D (prechodne): jednorazova sync ERROR — FIX v kodu + deterministicky IT, zbyva jen pasivni log-watch.** `MailSyncService: Critical error during folder sync INBOX: failed to create new store connection` (2026-06-25 11:17). Root cause: stavajici `imapRetryTemplate` obaluje JEN `store.connect()` ([ImapConnectionManager.java:425](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/ImapConnectionManager.java)), ale Angus `IMAPStore` tvori nove protokolove spojeni az BEHEM operaci nad slozkou (fetch/flag-sync), takze transient blip mid-cycle retry minul a spadl do catch-all jako ERROR+`last_error` bez retry. **Opraveno:** bounded retry+backoff kolem cele folder cyklu v [MailSyncService.performFullSyncCycle](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/MailSyncService.java) — transient (klasifikator [TransientMailErrors](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/TransientMailErrors.java), zarovnany s `RetryConfig`) se loguje WARN, dropne spojeni (`invalidateConnection`) a retryne; teprve vycerpany retry zapise `last_error`. Reuse `mail.client.retry.*`. Testy: [TransientMailErrorsTest](backend/src/test/java/org/voxrox/mailbackend/feature/mail/service/TransientMailErrorsTest.java) + 2 v [MailSyncServiceTest](backend/src/test/java/org/voxrox/mailbackend/feature/mail/service/MailSyncServiceTest.java) (retry→success, exhaust→record), pass-through v [ImapFolderExecutorTest](backend/src/test/java/org/voxrox/mailbackend/feature/mail/service/ImapFolderExecutorTest.java) (`TransientImapException` proleti exekutorem nezmenena az do retry smycky) a deterministicky [MailSyncTransientRetryGreenMailIT](backend/src/test/java/org/voxrox/mailbackend/feature/mail/service/MailSyncTransientRetryGreenMailIT.java) — fault injection (`@MockitoSpyBean MessageDownloader` hodi prvni `syncNewMessages` Angus blip, pak deleguje na realnou metodu) proti zivemu GreenMail + realnemu connection poolu/SQLite: cyklus se zotavi (overeno v logu: WARN+reconnect, 2 zpravy stazene, `last_error == null`). `mvn verify` zelene. **Zbyva:** nedeterministicky `tauri:dev` smoke nahrazen vyse uvedenym IT (transient blip nelze na zavolani vyvolat); jako produkcni signal zustava uz jen pasivni log-scan tripwire v [backend/RELEASE_CHECKLIST.md](backend/RELEASE_CHECKLIST.md) §8.
...
- [x] **Produkcni Google OAuth credentials** — HOTOVO 2026-06-23 (verifikovano pres Google Console). Produkce uz bezi na spravnem **org-based** clientu: `VoxRox Mail` (Desktop), client-id `37842026466-…`, v projektu **`VoxRox`** (`lucky-outpost-495609-j5`) pod organizaci **`voxrox.org`** (info@voxrox.org). Zapojeny lokalne v `backend/.env` i v CI secrets `GOOGLE_OAUTH_CLIENT_ID`/`GOOGLE_OAUTH_CLIENT_SECRET` (set 2026-06-17), odkud se peku do launcher cfg ([package-sidecar-windows.ps1:137-141](backend/scripts/package-sidecar-windows.ps1), [windows-signed-release.yml:54-55](.github/workflows/windows-signed-release.yml)). **Nerotuje se** — client uz je in use (rozhodnuto 2026-06-23); git-history secret scan byl cisty (viz nize). MS = public client, zadny secret. Pozn.: dev client `Google OAuth Test`/`TauriMailClient` (`462204841115-…`) v osobnim gmailu `luke.lacina@gmail.com` je pozustatek — nikde zadratovany neni, volitelny budouci uklid (osobni ucet).
  ...
  - **Consent screen HOTOVY 2026-06-23** (projekt `VoxRox`, authuser=1): app name `VoxRox Mail`, support email `info@voxrox.org`, **logo** (embossed mark z `C:\dev\java\brand\voxrox-mark.svg` → vyrenderovano do `voxrox-mark-512.png`), home `https://voxrox.org`, privacy `https://voxrox.org/privacy` (zije); **terms odkaz odebran** (vedl na 404 `https://voxrox.org/terms`); authorized domain `voxrox.org`; scopes `email`/`profile`/`openid` + restricted `https://mail.google.com/`.
  - **Zbyva (samostatne, First Release Gate):** projekt `VoxRox` je zatim **Testing** (External, test useri vc. kolegy) → pro verejnost potreba **Publish + CASA verifikace** (Tier 2, per-projekt, placene/rocni) kvuli restricted scope `https://mail.google.com/` ([application.properties:90](backend/src/main/resources/application.properties)). To je vetsi blok nez creds a patri k "Produkcni OAuth readiness". Alternativa = zustat Testing (≤100 test-useru) pro uzavrenou betu bez CASA.
...
- [x] **Zostrena globalni CSP — HOTOVO 2026-06-24.** V [tauri.conf.json](frontend/src-tauri/tauri.conf.json) (oba bloky `csp` i `devCsp`, parita drzena `check:csp`): pridan explicitni `script-src 'self'` a odstranen nepouzity `customprotocol:` placeholder (`default-src` ted `'self' asset:`). `tauri:dev` smoke overen 2026-06-23 pres CDP. **`tauri:build` smoke overen 2026-06-24** novym opakovatelnym skriptem [tauri-csp-build-smoke.mjs](frontend/scripts/tauri-csp-build-smoke.mjs) (`npm run tauri:smoke:csp-build`): packaged `app.exe` spusten s WebView2 remote-debuggingem, pres CDP injektnut `securitypolicyviolation` listener + reload na `http://tauri.localhost/` → inline bootstrap probehl (`__sveltekit_*` global pritomen), SPA se zhydratovala (content div ma deti), **0 CSP violations**. Tim potvrzeno, ze build-time hash-injection inline bootstrapu funguje (jiny mechanismus nez dev, ktery `check:csp` parita nepokryje).
