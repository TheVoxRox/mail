...
- [ ] **Re-verifikace auditu B1 (IMAP/SMTP) — 21 commitu driftu.** `npm run check:audits` (pridano 2026-08-08) ukazalo, ze od auditovaneho commitu `35a06f3` sahlo do `feature/mail/service` 21 commitu: threading phase 2, draft lifecycle, send-path review, rozpady sluzeb. Drift je zatim jen **zaevidovany** v [docs/audit-freshness.json](docs/audit-freshness.json), ne provereny — full-tier audit s takovouhle historii nemuze jit do release bez precteni. Mensi drift maji i B4 (+4), B5 (+2) a B2 (+1); u nich note v ledgeru popisuje, proc verdikt nejspis nehybe, ale potvrzeni take chybi.
- [ ] **Audit B6 nema platnou kotvu.** `Audited commit` v [docs/UPDATER_AUDIT.md](docs/UPDATER_AUDIT.md) je `e2b8d8d`, coz v repu neexistuje (`git cat-file -t` fail) — nejspis pre-squash SHA z vetve. Audit tim padem nejde overit proti zadnemu znamemu stavu kodu a jeho drift se neda spocitat. Kandidat na nahradu: `3162e6a` (#144, release-channels prace, kterou change log v1.2 popisuje) — potvrdit a opravit hlavicku.
