...
- [ ] **D (prechodne): jednorazova sync ERROR — pasivni watch.** `MailSyncService: Critical error during folder sync INBOX: failed to create new store connection` ([MailSyncService.java:220](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/MailSyncService.java), 2026-06-25 11:17). Transientni IMAP hiccup; dalsi cyklus se sam zotavil, ale catch-all loguje ERROR + kratce nastavi `last_error` (zadny retry). **Tripwire:** grep `failed to create new store connection` v `%LOCALAPPDATA%\VoxRox\Mail\logs\mail.log` (+ rotovane `mail.log.*.gz`) — **≤ ~1/den = sum, ignoruj; shluk nebo >par/den = eskaluj** na bounded retry+backoff a transient preklasifikuj na WARN. Pri vyskytu zaznamenat: shluk po sleep/wake nebo zmene site? provider/slozka? zotaveni v dalsim cyklu? Skenuje uz log-scan gate v [backend/RELEASE_CHECKLIST.md](backend/RELEASE_CHECKLIST.md) §8 (D je tam vyjmenovany).
