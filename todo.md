...
- [ ] **Frontend overrides v [frontend/package.json](frontend/package.json) — chybely removal conditions, doplneno 2026-08-07.** Kazdy je pin bez data expirace; historie ukazuje, ze pin sam casem spadne do zranitelneho rozsahu (`js-yaml` 4.3.0, `postcss` 8.5.12, `brace-expansion` 2.1.2/5.0.7 — vsechny tri se to uz staly). Stav k 2026-08-07 (`npm audit` cisty na vsech severitach):
  ...
  - `postcss: 8.5.25` — **override je uz zbytecny a navic skodi**: vsichni konzumenti (`vite ^8.5.23`, `postcss-load-config >=8.0.9`, `svelte-eslint-parser ^8.4.49`, …) maji caret rozsahy, takze npm by sam vzal nejnovejsi 8.x — dnes 8.5.26. Pin tedy drzi postcss o patch **zpatky**. Odstranit.
...
- [ ] **`OpenApiSnapshotTest` je nedeterministicky.** Dva po sobe jdouci `mvn -Dopenapi.snapshot.update=true -Dtest=OpenApiSnapshotTest test` nad nezmenenym stromem daly ruzne poradi `passwordPresentForNewAccount` a `providerOrCustomServerConfigPresent` ve schematu AccountRequest (overeno 2026-08-06). Gate tim muze zcervenat na cistem checkoutu a regenerace pridava do commitu nesouvisejici sum — oboji uci lidi re-pinovat snapshot bez cteni diffu. Fix: v `canonicalOpenApi` rekurzivne seradit klice pred zapisem i porovnanim; overit dvojim behem update (druhy nesmi soubor zmenit).
