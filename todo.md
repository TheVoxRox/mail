...
- [ ] **Podpisy zprav (per ucet)** — zajem potvrzen, az bude prostor. Rozsah: (1) ulozeni textu podpisu per ucet (backend `AccountEntity` + migrace, nebo nejdriv jen FE setting), (2) editace v nastaveni uctu (`settings/accounts/[id]`), (3) vlozeni do compose body ([ComposeForm.svelte:44](frontend/src/lib/components/ComposeForm.svelte) `body` state) — u nove zpravy na konec, u reply/forward konfigurovatelne nad/pod citaci (jako Outlook/TB). Zatim plain-text; az pristane HTML compose, podpora HTML podpisu. Vazba na [[project_desktop_app]] a HTML-compose polozku.
