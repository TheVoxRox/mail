...
- [ ] **Zostrena globalni CSP — kod HOTOVY, `tauri:dev` smoke OK, zbyva `tauri:build` smoke.** V [tauri.conf.json](frontend/src-tauri/tauri.conf.json) (oba bloky `csp` i `devCsp`, parita drzena `check:csp`): pridan explicitni `script-src 'self'` a odstranen nepouzity `customprotocol:` placeholder (`default-src` ted `'self' asset:`). **`tauri:dev` smoke overen 2026-06-23 pres CDP: appka se normalne nacte + zhydratuje, 0 CSP violations v konzoli** (silny dukaz, ze hash inline bootstrapu se injektuje OK). Zbyva stejny smoke v `tauri:build` (jiny hash-injection mechanismus, nejde overit v dev).
