...
- [ ] **Projit e2e sadu na vzor „`.focus()` a hned `keyboard.press`".** Tyz den shodily CI dva flaky teze tridy: klavesa odesla driv, nez si appka prevzala fokus. [row-actions-menu.functional.e2e.ts](frontend/src/routes/mail/row-actions-menu.functional.e2e.ts) — bits-ui usadi roving fokus az `afterTick`, klavesa do toho okna menu zaklini (#283). [list-navigation.functional.e2e.ts](frontend/src/routes/mail/list-navigation.functional.e2e.ts) — otevreni zpravy si samo bere fokus do iframu s obsahem, takze `Delete` dorazi do detailu a smaze **otevrenou** zpravu misto zaostreneho radku (#284). Opravene jsou jen tyhle dva soubory, vzor je v sade dal bezny. Pravidlo, ktere z toho vyslo: pred klavesou potvrdit, **kde fokus je**, u prochazek pres bits-ui menu navic drzet `KEY_PACE_MS`, a nebrat „prvek je videt" jako „appka je pripravena". Pozor pri overovani: oba flaky se lokalne reprodukuji spatne az vubec (`--repeat-each` nad celym souborem je minimum), takze u opravy casto nepujde ukazat pad pred a pruchod po — pak to tak i napsat, misto tvrzeni o regresi.
