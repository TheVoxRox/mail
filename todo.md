...
**Stav: implementace lezi NEZACOMMITOVANA v pracovni kopii. Vsech 6 P1 a 6 P2 nalezu z code-review (xhigh) je opraveno vcetne testu (2026-08-05), stejne tak vsech 5 chybejicich testu a 5 z 6 P3 (perf cross-folder dotazu, sdileny `sweepByIdCursor`, `folderLabelByRef`, `MailDraftService` sdili detekci markeru se `SubjectNormalizer`, cap `MAX_MARKERS` pryc). Zbyva posledni P3 polozka nize, ktera muze jit do naslednika. Gate na commit je tim splneny; PR je [#221](https://github.com/TheVoxRox/mail/pull/221).**
