...
- [ ] **Instalace nema progress ani informaci, ze se aplikace ukonci.** [lib.rs:224](frontend/src-tauri/src/lib.rs:224) zahazuje oba callbacky (`download_and_install(|_, _| {}, || {})`); dialog drzi staticke „Aktualizuji…" po celou dobu stahovani instalatoru s celym JRE, oba buttony disabled, pro screen reader uplne ticho. Progress callback poslat do stavu (`progressbar` + `role="status"`) a do textu v [cs.json](frontend/src/lib/i18n/messages/cs.json) doplnit, ze se aplikace zavre.
...
- [ ] **Selhani updatu se ohlasi dvakrat a ve dvou modalech nad sebou.** [UpdateFailureDialog.svelte:22](frontend/src/lib/components/UpdateFailureDialog.svelte:22) pushne toast s `update.failure.title`, coz je tentyz string jako titulek dialogu, ktery se otevira soucasne (e2e ocekava oboji). Navic prompt se pri selhani vraci na `available`, takze failure dialog stoji nad porad otevrenym promptem. Zrusit jedno z ohlaseni a prompt pri selhani zavrit.
...
- [ ] **Script injection v obou workflow.** [beta-channel.yml:62](.github/workflows/beta-channel.yml:62) interpoluje `${{ github.event.release.tag_name }}` primo do `run:` bloku, [windows-signed-release.yml:70](.github/workflows/windows-signed-release.yml:70) totez s `inputs['release-tag']` do pwsh stringu. Git refname pravidla povoluji `` ` ``, `$`, `;`, `&`, `|` i uvozovky, takze tag `v1.0.0";evil` je spustitelny kod s `contents: write` tokenem. Riziko nizke (vyzaduje write pristup), fix mechanicky: hodnotu do `env:` a cist `"$TAG"` / `$env:TAG`.
- [ ] **Vyber artefaktu pro `latest.json` je skore, ne tvrzeni.** [generate-tauri-latest-windows.mjs:57](frontend/scripts/generate-tauri-latest-windows.mjs:57) seradi podepsane kandidaty heuristikou a vezme prvni; shoda verze v nazvu je jen +10 bodu, ne podminka. Na cistem runneru vyjde vzdy, lokalne se starym artefaktem v `bundle/` tise vyrobi manifest na jiny build. Pridat tvrdy assert, ze nazev obsahuje `version`.
- [ ] **Drobnosti updateru** (jedna davka):
  - ~~guard v `installPromptedUpdate` propousti stav `installing`~~ — **HOTOVO 2026-08-27** pri oprave bodu 2: se stahovanim pred instalaci uz to nebyl jen tvar podminky, ale druhe stazeni, takze guard bere jen `available` a hlida to test.
  - `showMockUpdateFailureForTests` ([updates.ts:167](frontend/src/lib/updates.ts:167)) je vystaveny do `__MAIL_E2E__`, ale zadny test ho nevola — failure cesta jede pres `showMockUpdateForTests(..., { failInstall: true })`.
  - [build.rs](frontend/src-tauri/build.rs) neemituje `cargo:rerun-if-env-changed=TAURI_UPDATER_BETA_ENDPOINT`, takze `option_env!` na [lib.rs:149](frontend/src-tauri/src/lib.rs:149) muze z cache zapect starou hodnotu (projevi se jen lokalne, ne v CI).
  - `npm run tauri:updater-config` / [prepare-tauri-updater-config.mjs](frontend/scripts/prepare-tauri-updater-config.mjs) nevola zadny workflow ani skript; jedina zminka je v [WINDOWS_SIGNING.md](frontend/docs/WINDOWS_SIGNING.md). Bud rucni cesta patri do komentare skriptu, nebo je to reziduum.
  - Informativni poznamka z [UPDATER_AUDIT.md](docs/UPDATER_AUDIT.md) §6 (CI lint, ze `dangerousInsecureTransportProtocol` neni `true`) je otevrena od verze 1.0 (2026-07-09).
- [ ] **Revize [UPDATER_AUDIT.md](docs/UPDATER_AUDIT.md) po opravach** (uz tri potvrzeni driftu v [audit-freshness.json](docs/audit-freshness.json), cap je 8). Verdikt PASS se nemeni (zadny nalez nesaha na overovani podpisu), ale text sedet prestal na trech mistech: §1 enumeruje **dva** prikazy, kdyz jsou tri (`download_pending_update` pribyl), a tvrdi, ze install nebere argumenty — `expectedVersion` bere uz od v1.2; §2 tvrdi „fail-closed pipeline" o pipeline, ktera do 2026-08-27 shodu klicu neoverovala; injection plocha obou workflow v auditu neni vubec, prestoze oba ma v `Code paths`. Do `Code paths` navic patri [verify-updater-signature.mjs](frontend/scripts/verify-updater-signature.mjs) + [minisign.mjs](frontend/scripts/lib/minisign.mjs). Postup dle [docs/AUDIT_GUIDE.md](docs/AUDIT_GUIDE.md): audit + version bump + change log, radek v [SECURITY_THREAT_MODEL.md](SECURITY_THREAT_MODEL.md), verdikt v [backend/SECURITY_RELEASE_CHECK.md](backend/SECURITY_RELEASE_CHECK.md).
