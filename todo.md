...
- [ ] **Stavove automaty `ConversationList` do `.ts`, aby na ne slo napsat unit testy.** Je to nejdelsi soubor frontendu, ale delka sama problem neni — **zmereno 2026-08-19: z 1120 radku skriptu je 374 komentar, tedy tretina**, a ten komentar je tam pravem. Problem je **kde ta logika lezi**: vitest bere jen `src/**/*.test.ts` a komponentove testy repo nema zadne, takze uvnitr `.svelte` se na ni nedosahne; navic `eslint.config.js` vypina type-aware pravidla (`no-floating-promises`, `await-thenable`) prave pro `.svelte`, a tenhle soubor ma nejvic `void`/`await` mist v aplikaci. Dva soudrzne shluky, oba presne toho druhu, ktery unit test chyti a e2e ne: **(a) cache clenu vlakna** — `members` / `memberRequests` / `membersToken` / `membersPage` / `loadingThreads` / `expanded`, `loadMembers` ([ConversationList.svelte:421](frontend/src/lib/components/ConversationList.svelte)), `visibleMembersOf`, `refreshExpandedMembers`; je to generacni citac + sdileni in-flight requestu + invalidace podle identity stranky, tedy zavodni podminky. **(b) dvouurovnovy vyber** — `selected` / `selectedMembers`, `toggleConversation`, `toggleMember`, `conversationChecked`, `conversationMixed`, `pickedMembersOf`, `clearSelection`, `handleSelectAll`; invariant „konverzace nikdy neni v obou zaroven" a stav `mixed` se dnes daji overit jen proklikanim. Cil `lib/mail/threadMembers.svelte.ts` a `lib/mail/conversationSelection.svelte.ts`, odhad −350 radku a dve nove sady. **Cena, se kterou pocitat:** `loadMembers` cte `$conversationsState`, takze pres hranici modulu se store musi injektovat thunkem, stejne jako `element: () => …` u `createRovingGrid`. Vzeslo z analyzy frontendu 2026-08-19; ta same davka uz zavrela sdileni klikaci a ohlasovaci logiky mrizek (viz [CHANGELOG.md](CHANGELOG.md)). Sablonu **nedelit** — vetve `{#if isConversation}` uvnitr bunek jsou to, co drzi tracky zarovnane, rozpad radku na dve komponenty by obnovil vadu z #288.
- [ ] **Sdileny radek pro MessageList a SearchResultsGrid.** Repo uz sdili _mechanismy_ (`rovingGrid`, `MessageFlags`, `MessageRowActionsMenu`, `ui/focus-ring`, `mail/messageStatus`, `formatters`), ale sablona radku je opsana: bunky, jejich umisteni, tracky. Duvod pro _rozdily_ zapsany je (`SearchResultsGrid`: „there is no select column (search has no bulk mode)", k tomu sloupec slozky a `formatNumericDate` misto `formatMessageListDate`); duvod pro duplikaci samotne znacky nikde neni a git nepomuze — oba soubory prisly v uvodnim importu 01d8c1f. **Ze to stoji, je zmerene 2026-08-18:** tutez opravu na subgrid jsem musel udelat dvakrat a tentyz komentar napsat dvakrat, totez podlahu `min-h-8`, a ve vysledcich hledani mezitim **uplne chybel aktivovatelny prvek** — presne to, co se pri opisovani radku zapomene (doplneno tyz den jako `<button>`). Rozsah: **ne jeden grid** — sady sloupcu se lisi a ARIA indexy jsou konstanty kazde mrizky. Jednotka je sdileny _radek_ (nebo aspon modul s definici tracku a `grid-cols-subgrid`), kteremu se rekne, ktere bunky vykreslit. Pozor: pristupnostne kriticky kod se dvema vlastnimi e2e sadami, delat vedome, ne jako privazek. Souvisi s polozkou o brane na `grid-cols-[` vyse — sdileny radek by ji zuzil na jedno misto.
