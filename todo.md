...
- [ ] **jackson-databind 2.x — CVE-2026-54515 (watch — ZADNY 2.x fix, v appce netriggerovatelne).** Tranzitivni bridge dep: Spring Boot 4.1.0 tahá `com.fasterxml...:jackson-databind:2.21.4` vedle primarni (uz fixed) 3.x `tools.jackson:jackson-databind:3.1.4`. **PREHODNOCENO 2026-07-07:** v 2.x linii **zadna opravena verze NEEXISTUJE** (vsechny rozsahy open-ended, jediny `fixed` event je 3.x 3.1.4 = uz pouzivame); 2.21.5/2.22.0 OSV **stale flaguje**, override overen a revertnut (nalez nezmizi). **Reziduum je tady netriggerovatelne** (`ACCEPT_CASE_INSENSITIVE_PROPERTIES` neni zapnuta, `@JsonIgnoreProperties` jen Hibernate hygiena, primarni JSON stack jede na 3.x). **Akce:** zadna — nechat jako watch, dokud nevyjde opraveny 2.x release (`jackson-bom` s `fixed` eventem), pak `<jackson-2-bom.version>` override + `mvn verify`. CI OWASP DC failuje az na CVSS>=8. Detail viz [[reference_jackson_cve_2026_54515]] + [[reference_local_cve_scan_osv]].
