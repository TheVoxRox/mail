...
- [ ] Threading Phase 2 (V0.2) — UI grouping toggle, thread row aria-tree, bulk akce, a11y pass. References-only orphan reconciliation HOTOVO (junction tabulka `message_reference` + V2 migrace, rozsirena late-reconciliation o indexovany references lookup + backfill pass — viz CHANGELOG). Detail [backend/docs/THREADING_DESIGN.md](backend/docs/THREADING_DESIGN.md).
