...
- [ ] **Mrtvy kod na backendu: zbyva prohledavani volajicich, reziduum po prejmenovani a duplicitni testy.** Z davky 2026-08-24 jsou hotove dve brany (`-Xdoclint:reference` a `knip:production`, viz [CHANGELOG.md](CHANGELOG.md)); tyhle tri ne. **(a) Callerless sweep nad Javou — HOTOVO 2026-08-24**, `check:java-callers` v `npm run check`. Tri verdikty (dead / test-only / internal), vyjimky pres `@callerless <duvod>` u deklarace. Uklid, ktery to vyneslo, je v [CHANGELOG.md](CHANGELOG.md); nejzajimavejsi nalez nebyl mrtvy kod, ale tri testy s `verify(never())` na metodu, kterou nevolal nikdo — tvrzeni, ktere nemohlo selhat. **(b) Reziduum v diffu, ne v repu — HOTOVO 2026-08-24**, `check:rename-residue` v CI. Overeno proti realne historii: pusteno na `c947dea` najde `countByAccountIdGroupedByLabel` (tedy presne nalez, ktery se rucne opravoval az v #307) a jedno reziduum, ktere v repu lezelo dodnes. Nechytne `@DisplayName` retezce s `findByAccountId` — to jmeno porad deklaruje jiny repository, a podle jmena to rozhodnout nejde. **(c) Testy** — detekce identickych tel testovacich metod (`sameEmailOtherAccountAllowed` byla bajt po bajtu kopie souseda) a `@Disabled`/`.skip`/`.only` bez zduvodneni. Mutacni testovani (PIT) na „test, ktery nic netvrdi" zamerne **ne jako brana**, nanejvys obcasny sweep — burn-down by byl vlastni projekt.
