...
- [ ] **„Pozdeji" a Esc znamenaji „tuhle verzi uz nikdy".** [updates.ts:137](frontend/src/lib/updates.ts:137) persistuje verzi a [updates.ts:90](frontend/src/lib/updates.ts:90) ji pri startu navzdy preskakuje; `handleOpenChange` dela totez pri zavreni Escapem nebo klikem mimo, bez jakehokoli signalu. Bud tlacitko prejmenovat na „Preskocit tuto verzi", nebo persistovat az explicitni skip a „Pozdeji" nechat jen zavrit dialog. Chovani fixuje e2e `update prompt … Pozdeji dismissne verzi` v [boot.functional.e2e.ts](frontend/src/routes/boot.functional.e2e.ts) — zmena znamena upravit i ten test.
