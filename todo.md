...
- **`distributionSha256Sum` se do maven-wrapperu nedoplnuje.** Rozhodnuto 2026-08-17 pri revizi #279. Pin by hlidal jeden artefakt (`apache-maven-*-bin.zip`, ktery si stahuje CodeQL autobuild) — jenze jde po HTTPS z `repo.maven.apache.org`, tedy ze **stejneho originu jako vsechny ostatni zavislosti pomu**, ktere pinovane taky nejsou; chranil by tak zlomek plochy. Praktickych prekazek je navic dost: Maven Central pro tu zip publikuje `.sha512` a `.sha1`, ale **ne `.sha256`**, ktery ta property vyzaduje (overeno HTTP kody 2026-08-17), takze hodnotu by bylo nutne spocitat z vlastniho stazeni. A neni overene, ze dependabot ten hash pri bumpu aktualizuje — pokud ne, kazdy dalsi bump wrapperu shodi `Analyze (java-kotlin)`, coz je required check. Ozivuje se, kdyby se zaclo pinovat systematicky (cely pom), ne jen tenhle jeden soubor.
...
Obe polozky vznikly 2026-08-17 pri odblokovavani dependabot PR — jsou to zobecneni
uz opravenych konkretnich pripadu, ne nove nalezy.
...
- [ ] **Brana na eol drift: blob musi odpovidat deklaraci v `.gitattributes`.** Sourozenec `check:nul` a stejna trida tiche vady — soubor projde formatterem i testy, ale git ho vidi jinak, nez rika deklarace. Dependabot commituje pres GitHub API, ktere `.gitattributes` **neaplikuje**, takze umi ulozit blob s CRLF tam, kde `*.cmd text eol=crlf` rika LF; stalo se v #279 (`backend/mvnw.cmd`), sprava byla `git add --renormalize` (#285). Dopad: kazdy klon hlasi soubor trvale jako modified se 189radkovym diffem, ktery **nejde uklidit** `checkout --` ani `reset --hard`, protoze smudge ho ze spatneho blobu vyrobi znovu. Detekce je `git ls-files --eol` (porovnat `i/` proti `attr/`); `git status` prukazny NENI — cerstve vypsany soubor git obsahove neporovnava (stat cache), takze checkout do odhozeneho worktree vyjde cistotne i u vadneho blobu. Gate patri mezi `check:*` ve [frontend/scripts/](frontend/scripts/) a podle konvence repa s vlastni sadou proti odhozenemu git repu. Dependabot bezi tydne, takze se to bude opakovat.
