...
- [ ] **CI gate na shodu podpisoveho paru.** Mezi `TAURI_UPDATER_PUBKEY` (config) a `TAURI_SIGNING_PRIVATE_KEY` (`.sig`) neexistuje automaticka vazba; pri rozjezdu workflow dobehne zelene a **kazda instalace prijde o updater**. Tauri `.sig` je base64 minisign signatury a `pubkey` base64 minisign klice, takze krok po `tauri:generate-latest:windows` v [windows-signed-release.yml](.github/workflows/windows-signed-release.yml) muze oboji dekodovat a overit instalator (`minisign -Vm`). Jedina kontrola, ktera z neopravitelneho rizika udela build gate — ma prednost pred zbytkem teto sekce.
- [ ] **Instalator nekontroluje bezici sidecar.** [nsis-installer.nsi:640](frontend/src-tauri/windows/nsis-installer.nsi:640) vola `CheckIfAppIsRunning` jen na `${MAINBINARYNAME}.exe`. Jpackage sidecar je samostatny proces s vlastnim jlink runtime a `runtime/` + `app/` jsou Tauri resources v temze `$INSTDIR` ([tauri.conf.json](frontend/src-tauri/tauri.conf.json)), ktere instalator prepisuje pres `File /a`. Plugin spousti instalator a teprve pak konci app proces, takze watchdog (`MAIL_SIDECAR_WATCH_PARENT`) okno jen zuzuje — je to zavod, ne synchronizace. Neoverene staticky; bud vlastni bod update smoke, nebo pridat tutez kontrolu na sidecar launcher do `.nsi`.
...
- [ ] **Drobnosti updateru** (jedna davka):
  - [updates.ts:124](frontend/src/lib/updates.ts:124) — guard propousti i stav `installing`, takze druhe volani spusti druhy `install_pending_update`. Dnes nedosazitelne (button disabled), ale podminka ma tvar opaku toho, co chce rict.
...
- [ ] **Revize [UPDATER_AUDIT.md](docs/UPDATER_AUDIT.md) po opravach.** Verdikt PASS se nemeni (zadny nalez nesaha na overovani podpisu), ale §2 dnes tvrdi „fail-closed pipeline" o pipeline, ktera shodu klicu neoveruje, a injection plocha obou workflow v auditu neni vubec — a oba workflow ma v `Code paths`. Postup dle [docs/AUDIT_GUIDE.md](docs/AUDIT_GUIDE.md): audit + version bump + change log, radek v [SECURITY_THREAT_MODEL.md](SECURITY_THREAT_MODEL.md), verdikt v [backend/SECURITY_RELEASE_CHECK.md](backend/SECURITY_RELEASE_CHECK.md).
