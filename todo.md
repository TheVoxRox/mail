...
- [ ] **E (stredni): duplicitni OAuth callback → matouci "selhalo" stranka i po uspesnem pridani.** Pri pridani Google uctu (2026-06-25, fresh install): dva OAuth flow (pravdepodobne dvoji trigger Google/Pridat-ucet tlacitka) → autorizacni requesty se v session prebily; prvni callback uspel (ucet vytvoren + 174 zprav synced, requiresReauth=false), druhy nasel spotrebovany request → `authorization_request_not_found` → system prohlizec ukazal `auth-failed.html?reason=authorization_request_not_found` ("Prihlaseni se nezdarilo"). App SAMA se zotavila (#66 polling nasel ucet, zpravy vid"), ale prohlizec mate. Fix kandidati: debounce/guard Google tlacitka (nepustit 2 flow), NEBO zrychlit callback (account-creation + crypto self-test ~5s drzi okno otevrene), NEBO auth-failed.html pro `authorization_request_not_found` zmekcit (ucet uz mohl byt pridan — zkontroluj appku). Vazba na [[project_microsoft_oauth_azure]] / OAuth flow.
- [ ] **G (perf/log-hygiene): Hibernate HHH90003004 spam pri listu zprav.** ~20× `firstResult/maxResults specified with collection fetch; applying in memory` pri nacitani stranky zprav — strankovani s collection JOIN FETCH dela Hibernate v pameti (neefektivni pro velke slozky). Najit dotaz (message list s fetchovanou kolekci, nejspis attachments) a oddelit strankovani od collection fetch (dva dotazy / @BatchSize / EntityGraph bez paginace kolekce).
