...
- [ ] **Mail TLS hardening (CodeQL `java/insecure-smtp-ssl`)** — ROZPRACOVANO 2026-06-18: SMTP [SmtpTransportFactory](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/SmtpTransportFactory.java) STARTTLS vetev nema `starttls.required` (tichy plaintext downgrade u heslovych uctu) + `checkserveridentity` spoleha na knihovni default. Fix: explicitni `ssl.checkserveridentity=true` + `starttls.required=true`, parita i pro IMAP [ImapConnectionManager](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/ImapConnectionManager.java) + [MailConnectionProbe](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/MailConnectionProbe.java). Pozn.: plaintext IMAP (`useSsl=false` → protokol `imap`) zustava moznym dle designu — vynuceni STARTTLS je samostatne produktove rozhodnuti. JS alerty (`js/missing-origin-check` MSW worker, `js/file-access-to-http` ×2 smoke skripty) jiz dismissnuty "used in tests" (dev-only, loopback).
...
- [ ] **Podpisy zprav (per ucet)** — zajem potvrzen, plan zrevidovan, ceka na prostor. Rozhodnuti: podpis vklada FRONTEND do compose body (uzivatel ho vidi a edituje pred odeslanim, jako Outlook/TB) — backend ho jen uklada/vraci, [MimeMessageBuilder](backend/src/main/java/org/voxrox/mailbackend/feature/mail/service/MimeMessageBuilder.java) se nemeni. Podpis NENI tajny → obycejny sloupec, zadne DPAPI. Oddelovac `-- \n` (RFC 3676). **Fazovat:** Faze 1 = jen nova zprava (podpis na konec); reply/forward placement az Faze 2 (kvuli `-- ` × citace).
  - Backend: migrace `V2__add_account_signature.sql` (`ALTER TABLE accounts ADD COLUMN signature TEXT`) — Flyway je jediny vlastnik schematu (`ddl-auto=none`, [application.properties:46](backend/src/main/resources/application.properties)). Pole `signature` do [AccountEntity](backend/src/main/java/org/voxrox/mailbackend/feature/account/entity/AccountEntity.java), [AccountUpdateRequest](backend/src/main/java/org/voxrox/mailbackend/feature/account/dto/AccountUpdateRequest.java) (`@Size(max=10000)`, REUSE `{validation.size.max}` — zadny novy i18n klic), [AccountResponse](backend/src/main/java/org/voxrox/mailbackend/feature/account/dto/AccountResponse.java) (OBA konstruktory), [AccountMapper](backend/src/main/java/org/voxrox/mailbackend/feature/account/mapper/AccountMapper.java) (map + apply update).
  - API typy: po zmene DTO `npm run generate:api && npm run check:api` (regen [schema.d.ts](frontend/src/lib/api/schema.d.ts)).
  - Frontend UI: textarea "Podpis" pres `Field` (label+hint, a11y) v editaci uctu ([settings/accounts/[id]/+page.svelte](frontend/src/routes/settings/accounts/%5Bid%5D/+page.svelte) / `AccountForm.svelte`) + i18n `settings.account.signature.*` (cs/en).
  - Frontend compose ([ComposeForm.svelte](frontend/src/lib/components/ComposeForm.svelte)): vlozit v `applyPrefill` ([:95](frontend/src/lib/components/ComposeForm.svelte)) jen pro novou zpravu/mailto, NE pro `?draft=` (jiz obsahuje podpis → duplicita; vetve viz [prefill.ts](frontend/src/lib/compose/prefill.ts)). Vlozeni MUSI byt pred [:121](frontend/src/lib/components/ComposeForm.svelte) (`lastSavedSnapshot`), jinak falesny unsaved-changes. Swap pri zmene `fromAccountId` ([:82](frontend/src/lib/components/ComposeForm.svelte)) pres EXPLICITNI state s vlozenym podpisem (nahradit jen pokud body ten blok stale obsahuje verbatim; rucne smazany nevracet). Logiku vytahnout do ciste funkce v `compose/` (unit test, knip-safe).
  - Edge: ucet bez podpisu = nevkladat nic; plain-text ted, az pristane HTML compose → HTML podpis sanitizovat na renderu (reuse [content-sanitizer](frontend/src/lib/mail/content-sanitizer.ts)). Vazba na [[project_desktop_app]] + HTML-compose polozku.
  - Testy: `AccountMapperTest` signature round-trip + migrace v `mvn verify`; FE unit pro vkladaci funkci (nova vs draft vs swap vs prazdny); a11y textarea.
  - Commity: (1) backend persist, (2) regen API typu, (3) FE settings UI, (4) FE compose vkladani+testy. Odhad ~1-1,5 dne.
