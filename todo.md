...
- [ ] **Backend overrides v [backend/pom.xml](backend/pom.xml) — cekaji na Spring Boot.** Docasne bezpecnostni piny; odstranit, az je Boot dozene. Kontrolovat pri kazdem Boot bumpu (4.1.0 = posledni k 2026-08-06, stale managuje vsechny ctyri stare verze):
  ...
  - `<tomcat.version>11.0.25</tomcat.version>` (CVE-2026-66299) — odstranit az managed `tomcat.version` >= 11.0.25. Boot 4.1.0 spravuje 11.0.22 (overeno 2026-08-19).
...
(2026-08-19) a je uz opravena; zbyla po ni jedna otevrena polozka nize (apt
fetch pri degradovanem mirroru).

- [ ] **Mrtvy kod na backendu: zbyva prohledavani volajicich, reziduum po prejmenovani a duplicitni testy.** Z davky 2026-08-24 jsou hotove dve brany (`-Xdoclint:reference` a `knip:production`, viz [CHANGELOG.md](CHANGELOG.md)); tyhle tri ne. **(a) Callerless sweep nad Javou** — ekvivalent toho, co knip dela pro TypeScript: pro kazdou metodu v `backend/src/main` najit volajici mimo vlastni soubor, s whitelistem se zduvodnenim pro zamerne pripady (`ImapCapabilities.hasQresync`, `OAuth2TokenService.exchangeRefreshToken`). Prototyp 2026-08-24 nad 460 kandidaty vratil 12 bez volajiciho a 19 volanych jen z testu — tedy seznam k oku, ne stovky; falesne pozitivy byly `@AssertTrue` validace a vnorene recordy, oboji jde do skip listu. **Oba konkretni nalezy prototypu uz jsou vyrizene** (2026-08-24): `InternalApiKeyProvider.invalidateCache` smazana — overeno, ze o chybejici invalidaci neslo, `HandshakeService` generuje klic jednou za proces do pameti a rotacni cestu nema, takze neni co invalidovat; `MessageDownloader.getLatestUidFromServer` **nebyla mrtva**, jen prilis viditelna (volajici ma ve stejnem souboru, radek 75) a je nove `private`. To je zaroven poucka pro brany podle bodu (a): „bez volajiciho mimo vlastni soubor" a „mrtva" nejsou totez a hlasit se to musi zvlast. **(b) Reziduum v diffu, ne v repu — HOTOVO 2026-08-24**, `check:rename-residue` v CI. Overeno proti realne historii: pusteno na `c947dea` najde `countByAccountIdGroupedByLabel` (tedy presne nalez, ktery se rucne opravoval az v #307) a jedno reziduum, ktere v repu lezelo dodnes. Nechytne `@DisplayName` retezce s `findByAccountId` — to jmeno porad deklaruje jiny repository, a podle jmena to rozhodnout nejde. **(c) Testy** — detekce identickych tel testovacich metod (`sameEmailOtherAccountAllowed` byla bajt po bajtu kopie souseda) a `@Disabled`/`.skip`/`.only` bez zduvodneni. Mutacni testovani (PIT) na „test, ktery nic netvrdi" zamerne **ne jako brana**, nanejvys obcasny sweep — burn-down by byl vlastni projekt.
- [ ] **Apt fetch v `Install Tauri system deps` pri degradovanem mirroru — bounded retry to jen zmirnuje, neresi.** Zmereno 2026-08-20 na behu `32295477243`: vsechny tri pokusy narazily na 200s strop, krok spalil 622 s a skoncil `exit 1` — tedy presne ten prechod ze „zelene, ale pomale" na „cervenou", proti kteremu se retry pridaval. **Diagnoza v puvodnim zapisu nesedi na to, co se stalo:** tvrdila, ze `Acquire::http::Timeout` nikdy nevystreli, protoze prenos postupuje (`Fetched 21.1 MB in 12min 38s`), jenze v tomhle behu apt po failoveru na `archive.ubuntu.com` uspesne stahl InRelease ve 20:04:48 a pak **162 s nerekl vubec nic** az do killu. To je mlcici spojeni, tedy presne pripad, na ktery `Acquire::http::Timeout` plati. Dve levnejsi paky, obe miri na skutecne pozorovanou pricinu: (a) `-o Acquire::http::Timeout=30 -o Acquire::Retries=3`, aby se mlcici spojeni utlo uvnitr jednoho pokusu misto celeho kroku; (b) **pripnout mirror, na ktery apt uspesne prepnul** — dnes kazdy pokus znovu plati ~38 s tim, ze odznova zjistuje, ze `azure.archive.ubuntu.com` je mrtvy (`Ign:` na vsech ctyrech repech, 20:04:10 → 20:04:48), tedy ~114 s z 600s rozpoctu na host, o kterem uz vime. K tomu dve drobnosti ze stejne review: retry neodlisuje exit 124 od skutecne chyby apt, takze 404 nebo prejmenovany balik spadne za 5 s a stejne se hlasi jako „did not finish within 200s" a dvakrat zopakuje; a `sleep 5` bezi i po poslednim pokusu. Vzeslo z code-review #301 2026-08-20.
