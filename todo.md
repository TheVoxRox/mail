...
- [ ] **Jednorazove overit expiraci Authenticode certifikatu Windows buildu (#170 nalez 5).** `windows-signed-release.yml` podepisuje jen kdyz existuje secret `WINDOWS_CERTIFICATE_BASE64` (bez nej jde installer nepodepsany; updater `.sig` je nezavisly a funguje dal). Z repa datum expirace nejde zjistit — overit, jestli je secret vubec nastaveny, a pokud ano, kdy certifikat vyprsi (klasicky duvod "za N mesicu prestal jit release"); zjistene datum zapsat sem.
