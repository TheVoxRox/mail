...
- [ ] **DPAPI `crypto.bin`** — cisty profil → start → `crypto.bin` zacina `VOXSEC1` a jeho bytes nejsou plaintext klic; pridat ucet → restart → sync/login funguje (credentials se desifruji); podstrcit *legacy* plaintext `crypto.bin` → start → overit in-place migraci na `VOXSEC1`.
