...
(2026-08-19) a opravena zatim neni.
...
- [ ] **`timeout-minutes` v [.github/workflows/ci.yml](.github/workflows/ci.yml) — zadny job ho nema, plati tedy vychozich 360 minut.** Jinde v repu se timeouty pouzivaji (`codeql.yml` 30, `vuln-scan.yml` 15/30/45/90), `ci.yml` je jediny bez nich. **Zmereno 2026-08-19 na #296:** joby, ktere na zacatku sahaji na sit — `Install Tauri system deps` (apt) a `Install Playwright browser` (`--with-deps`, tedy taky apt) — zatuhly na spojeni bez odpovedi; jeden vydrzel 80 minut proti normalnim ~6, a se zapnutym auto-merge se celou tu dobu ceka. Neni to deterministicke: tentyz job jednou visi a podruhe projde bez zmeny vstupu, a na soubezne bezicim #297 prosel za 6 minut — takze restart je obejiti, ne oprava, a skutecna obrana je ohraniceni. Pri volbe hodnoty pozor na strop, ne na median: nejpomalejsi legitimni joby jsou `Tauri (Rust)` ~6 min, `Backend quality` ~4 min, `Analyze (java-kotlin)` ~3,5 min. Zvazit u toho i `-o Acquire::Retries=3` primo v apt kroku. Diagnostika zaseknuteho jobu viz [[reference_ci_wedged_job_diagnosis]].
