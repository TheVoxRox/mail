...
- [ ] **Script injection v obou workflow** — nalez **U-1** v [UPDATER_AUDIT.md](docs/UPDATER_AUDIT.md) §6 (Low, otevreny). [beta-channel.yml:62](.github/workflows/beta-channel.yml:62) interpoluje `${{ github.event.release.tag_name }}` primo do `run:` bloku, [windows-signed-release.yml:70](.github/workflows/windows-signed-release.yml:70) totez s `inputs['release-tag']` do pwsh stringu. Git refname pravidla povoluji `` ` ``, `$`, `;`, `&`, `|` i uvozovky, takze tag `v1.0.0";evil` je spustitelny kod s `contents: write` tokenem. Riziko nizke (vyzaduje write pristup), fix mechanicky: hodnotu do `env:` a cist `"$TAG"` / `$env:TAG`.
