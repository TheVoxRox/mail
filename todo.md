...
- [ ] **Brana na navrat per-radkoveho gridu.** Tri komponenty (`MessageList`, `SearchResultsGrid`, `ConversationList`) drzi tytez sdilene tracky na kontejneru a `grid-cols-subgrid` na radcich, ale nic nebrani tomu, aby dalsi row-level komponenta zase napsala `grid-cols-[...]` primo na radek — a sloupce se rozejdou potichu, presne jako kdysi radiusy a fokusove prstence pred `check:design`. Navrh pravidla do [check-design.mjs](frontend/scripts/check-design.mjs): odmitnout `grid-cols-[` na elementu, ktery zaroven nese `role="row"`. Podle konvence repa s vlastni sadou proti odhozenemu git repu. Vzeslo z code-review 2026-08-18 jako altitude nalez: tri komentare se ctou hur nez jedna brana.
