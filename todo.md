...
- [ ] **Posledni rozhodnuti, ktere zamyka schema — udelat ho, dokud je `V1__init.sql` editovatelne.** Po prvnim release z nej je migrace nad uzivatelskymi daty. Zbytek DB analyzy (adresar mimo ucty, katalog provideru mimo migraci, drift gate, redundantni indexy, `NOT NULL`) je hotovy, detail v [CHANGELOG.md](CHANGELOG.md).
  - **`accounts.signature` nema format.** Podpisy Faze 2 pocitaji s HTML compose; pak bude potreba `signature_html`, nebo `signature_format` discriminator. Pridat prazdny sloupec ted je zdarma, pozdeji to je migrace + interpretace existujicich podpisu.
...
- [ ] **swagger-ui webjar nese zranitelny DOMPurify — ceka na upstream release (medium, code-scanning alerty #130/#131).** GHSA-55q2-fjhq-7xh7: po odstraneni IN_PLACE hooku zustane odpojeny podstrom spustitelny (XSS), postihuje DOMPurify <= 3.4.12. Hlasi to dependency-check nad `swagger-ui-bundle.js` a `swagger-ui-es-bundle.js` uvnitr `org.webjars:swagger-ui:5.32.11`, ktery si tahne springdoc. **Neni to exploitovatelne:** ten bundle v aplikaci nic nenacita, `springdoc.api-docs.enabled` i `springdoc.swagger-ui.enabled` jsou defaultne vypnute ([application.properties:241](backend/src/main/resources/application.properties)) a produkcni fat jar springdoc vubec neobsahuje. Samotny webjar (1,2 MB) ve fat jaru lezi — `excludeGroupIds` filtruje podle groupId a webjar ma `org.webjars` — ale jeho vyhozeni by alert stejne nezavrelo, protoze scanner cte resolvnuty dependency set, ne vysledny jar. **Fix zatim neexistuje:** DOMPurify je opraveny v 3.4.13, swagger-ui, ktery by ho nesl, jeste nevysel (Maven Central ma k 2026-08-10 jako latest porad 5.32.11). Akce: sledovat Maven Central a bumpnout pin `org.webjars:swagger-ui` v [backend/pom.xml](backend/pom.xml), jakmile takovy webjar vyjde. Rozhodnuto 2026-08-10, ze se zatim neprechazi na `springdoc-openapi-starter-webmvc-api` (varianta bez UI), ktera by zavislost odstranila natrvalo, ale pripravila dev o Swagger UI.
