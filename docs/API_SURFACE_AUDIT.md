...
and `include-stacktrace=never`, and `application.properties` is the only
configuration file the jar carries, so no profile can turn them back on. No
exception message, SQL, or stack frame reaches the client.
...
message counts, and JVM/runtime info. A folder the user created is named by a
