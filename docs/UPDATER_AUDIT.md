...
- The WebView holds **no updater plugin permissions at all**
  ...
  supply either.
- **Two further commands sit beside them and reach no updater state** (v1.14):
  `get_update_startup_check()` and `set_update_startup_check(enabled)` in
  [update_preference.rs](../frontend/src-tauri/src/update_preference.rs) read
  and write one DWORD, `UpdateStartupCheck` under
  `HKCU\Software\VoxRox\Mail[suffix]`, which the installer's privacy page also
  writes. They take a boolean or nothing — no path, URL, key or version — and
...
- **Startup check** ([bootstrap.ts](../frontend/src/lib/bootstrap.ts) →
  ...
  on by default; v1.13), and **once per process** (v1.11). The installer's
  privacy page asks the same question and records the answer in the registry;
  after the once-per-process guard the function adopts a recorded answer over
  the webview's value before it reads the setting (v1.14), because the WebView2
  profile survives a reinstall. The setting is read
...
- Covered by [updates.test.ts](../frontend/src/lib/updates.test.ts): fail-silent
  ...
  request at all, and a manual check still answers. v1.14 adds three: a
  recorded "off" wins over a stored "on", a recorded "on" over a stored "off",
  and with nothing recorded the stored value stays in charge;
  [updateStartupCheck.test.ts](../frontend/src/lib/stores/updateStartupCheck.test.ts)
  covers the write-back.
...
- **1.16** (2026-09-16) — **re-verified against `a2e6d43`, clearing all seven
...
- **1.14** (2026-09-14) — **no claim in the trust chain moved; verdict stays
  ...
  gains a privacy page that asks whether the application may check for updates
  when it starts, again for SignPath Foundation's terms. Two changes under
  `Code paths`. (1) `frontend/src-tauri/src` gains `update_preference.rs` and
  registers `get_update_startup_check` / `set_update_startup_check`, which §1
  now names beside the three updater commands: one DWORD under HKCU, a boolean
  or nothing as input, and no route to the pending or downloaded slots, the
  channel map or the pubkey. (2) `updates.ts` adopts the recorded answer after
  the once-per-process guard and before the setting, so §5's gate gains an input
  but never a request: a recorded `false` returns before `check_for_update`
  exactly as the unticked setting did, a recorded `true` restores the default,
  and nothing recorded changes nothing. `AboutSettings.svelte`,
  `capabilities/`, `tauri.conf.json` and both workflows are byte-identical. The
  NSIS template is outside `Code paths`; its page runs before any file is copied
  and has no part in how an update is verified or installed.
...
- **1.12** (2026-09-07) — **re-verification, no claim moved; verdict stays
  ...
  five, the next two acknowledgements would have tripped the warning and the
  eighth would have demanded exactly this work at whatever moment it happened
  to arrive. Nothing here was verified from the ledger notes — the whole drift
  under `Code paths` since the last anchor is seven files, and each claim was
  re-checked in the tree at `0165149`. §1: the handler still registers exactly
  `check_for_update`, `download_pending_update`, `install_pending_update`;
  `capabilities/` grants nothing under `updater:`; `beta_endpoint_override`
  still maps `stable` to `None`, `beta` to the compile-time constant and
  everything else to an error; a check still clears the pending slot **and** the
  downloaded package; both commands still take `expected_version` and compare it
  before anything runs, the install comparing the package's own version a second
  time and dropping it on mismatch; and the bytes still never cross into the
  renderer — `install(&package.bytes)` is Rust-side and the command returns
  `Result<(), String>`. §2: the release still builds with
  `--config src-tauri/tauri.release.conf.json`, "Validate Tauri updater signing
  secrets" still throws on an absent private key, `buildUpdaterPlugin()` still
  requires `TAURI_UPDATER_PUBKEY` and a non-empty endpoint list,
  `dangerousInsecureTransportProtocol` is still opt-in and unset, and the
  manifest generator still takes only a signed artifact, throws on an empty
  signature, requires the version as a whole token in the name and builds the
  URL with `encodeURIComponent`. §3: `allowDowngrades` is still `false` and
  `connect-src` is still loopback + `ipc:` with no `github.com`. §4: `{@html}`
  is still absent from `frontend/src`, count zero. §5: the once-per-process
  gate, the download → stop → install order, the stop flagged before its await
  and the failure dialog raised before the restart are all as v1.11 describes
  them. §6: U-1 re-checked mechanically rather than by eye — no `${{ }}` remains
  inside any `run:` block of either workflow, still true after the two steps
  that changed in this window (an `actions/setup-java` bump and a smoke-step
  rename). The one open item is unchanged and procedural: nothing in code stops
  a bare `npm run tauri:build` from being published, only RELEASE_CHECKLIST §3a.
